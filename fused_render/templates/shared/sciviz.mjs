/* ===========================================================================
 * sciViz core + UI kit — shared by geotiff_template.html, netcdf_template.html,
 * zarr_template.html. Ported 1:1 from app/src/ui/components/sci/sciViz.ts +
 * SciPreviewParts.tsx / SciHistogram.tsx / useCanvasHover.ts.
 *
 * Extracted (2026-07) after confirming, function-by-function, that all three
 * templates carried byte-identical copies of everything below. What did NOT
 * match across all three stayed in-template: CF-time labeling (`cfTimeLabel`
 * / `CF_TIME_MS`) and the leading-dimension slider (`makeDimSlider`) are used
 * by netcdf/zarr only (geotiff has no CF-time axis or leading dims); the RGB
 * composite helpers (`drawRgb`, `stretchToBytes`) are geotiff-only.
 *
