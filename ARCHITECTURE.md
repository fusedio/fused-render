...
│   ├── server.py               # FastAPI() construction + app.state wiring + include_router() only — no route/helper logic lives here
│   ├── _server_common.py       # logger/STATIC_DIR/_error/_require_fused, exception handler, access-log middleware, pooled-client + get_start_dir/get_shell_path deps
│   ├── _server_shell.py        # router: "/", "/view/{path}", "/embed/{path}" (serves the built React shell)
│   ├── _server_config.py       # router: /api/mounts/health, /api/config, /api/desktop/shutdown
│   ├── _server_session.py      # per-file sidecar + router: /api/session GET/PUT
│   ├── _server_templates.py    # template registry resolution + condition gating (dirs, matching, name resolution, icons, /api/fs/conditions payload)
│   ├── _server_gitignore.py    # .gitignore-aware walk oracle
│   ├── _server_walk.py         # directory listing + recursive walk primitives (_walk_bfs, _list_direct, ...)
│   ├── _server_mount.py        # stat cache + remote-mount probing/writability
│   ├── _server_watch.py        # router: /api/fs/events (WS) filesystem watch/poll registry
│   ├── _server_fs_read.py      # router: /api/fs/stat|conditions|list|walk|raw|events|reveal
│   ├── _server_fs_mutate.py    # fs mutation helpers + trash, router: /api/fs/write|mkdir|delete|rename|copy
│   ├── _server_proxy.py        # /api/fs/raw upstream proxy + response hardening
│   ├── _server_render.py       # router: /render
│   ├── _server_run.py          # router: /api/run
│   ├── _server_ai.py           # fused.ai: Claude CLI binary resolution, _AiSession, router: /api/ai
│   ├── _server_export.py       # router: /api/export
