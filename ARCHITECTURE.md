...
│   │       ├── selffix.py      # self-fix: /api/selffix start|read|clear — a Claude session on THIS install (SPEC §42/D306)
...
│   ├── selffix.py              # the modified-install mark: digest, marker under <install>/.fused-render-selffix, fix prompt (SPEC §42)
│   ├── static/
