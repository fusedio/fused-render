...
### Undo/redo covers the ops that are a rename both ways (D329)

- **FS-17** **`⌘/Ctrl+Z` (and `⇧⌘/Ctrl+Z`) undo the explorer's RELOCATIONS — a drag-move, a cut-paste, a rename, and the TRASH delete — and nothing else.** All four are one primitive at the filesystem: they rename a path, their inverse is another rename, it destroys nothing, and applying the inverse twice is the original op, so undo, redo and undo-again all run the same code (`lib/fs-undo`: an op is a list of `{from, to}` absolute-path pairs, never a closure; `UNDO_CAP` = 50 ops; a module-level in-flight guard, because a move can navigate mid-gesture and a guard living in a component would remount to `false`).
  **The trash delete is in the list because it IS a rename, not because the rule was relaxed.** `_move_to_trash` moves an entry by `os.rename(path, ~/.Trash/<deduped name>)` — the server picks the destination — and reports it as **`trashed_to`** on `/api/fs/delete`; the listing records the batch as one `delete` op, so a single `⌘Z` renames the whole selection out of `~/.Trash` back to where it was and `⇧⌘Z` renames it back in. The restore asks for the **exact recorded path with overwrite off**, so a name retaken since the delete is a 409 the toast says out loud rather than a silent restore as "… copy"; an emptied Trash is a 404 per pair, reported the same way; a systemic refusal (a read-only destination) stops the batch and leaves the untried pairs undoable.
  **This is a macOS, local-filesystem, this-session promise, and the spec says so in four cases rather than letting a reader generalise it:** (1) **macOS + local, same device** → `os.rename` into `~/.Trash`, destination known, **undoable**; (2) **macOS + cross-device** (or any other `OSError`) → the `osascript` Finder fallback, which is recoverable **in Finder** but **not undoable** — Finder chose the location, we cannot name it, and a pair with an unknown end must not be recorded; (3) **macOS + a remote mount-backed file** → a deliberate **501 `trash unsupported`** (a trash move would pull the whole file through the kernel), so it routes to the confirm-then-hard-delete flow and is **irreversible**; (4) **Windows and Linux** → `_trash_supported()` is `sys.platform == "darwin"`, so also **501** and also the confirm-then-hard-delete flow: there is no Recycle Bin or XDG-trash backend, and the file is **erased permanently**. Undoability is additionally **session-scoped** — the stack is in memory, so a reload or an eviction past `UNDO_CAP` leaves the entry in `~/.Trash` to be dragged out by hand — and, because the fast path is a bare `os.rename`, it writes none of Finder's own put-back record, so Finder's "Put Back" is not a second way home for entries this app trashed. Recents loses the entry either way (`notePathDeleted`), and the undo does not re-note it.
