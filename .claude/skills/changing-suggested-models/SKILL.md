...
- Trusting that unit tests asserting `mirror_id(id) == id` prove the permission wiring is correct. They don't, if `id` itself is untranslated — assert against what `GGUF_RECIPES` says the repo is, the same trap D421 records.
