...
def _harden_raw(resp, request: Request):
    ...
    if dest in _srv._DOCUMENT_DESTS or mode == "navigate":
        media = resp.headers.get("content-type", "").split(";")[0].strip().lower()
        if media in _srv._SCRIPTABLE_MEDIA:
...
def _proxy_raw(upstream: str, request: Request):
    ...
    try:
        ...
        out = {k: v for k, v in e.headers.items() if k.lower() in _srv._PROXY_HEADERS}
        try:
        ...
        return None
    out = {k: v for k, v in r.headers.items() if k.lower() in _srv._PROXY_HEADERS}
