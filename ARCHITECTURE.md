...
- **Error overlay:** module-level helper — on unhandled promise rejection carrying `.traceback` (i.e. a runPython failure the page didn't catch), render a fixed-position red-bordered overlay with type, message, and a `.where` headline (`<file>, line N, in <func>` + the source line — the failing line of the user's script) when known. The headline is the culprit; the full `<pre>` traceback tucks behind a collapsed `Traceback` toggle so it doesn't repeat the line the trimmed traceback now leads with. When there is no `.where` (harness error, timeout, missing file) the traceback is shown outright, since it is the only content. Author-handled rejections show nothing.
