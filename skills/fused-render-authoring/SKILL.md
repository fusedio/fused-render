...
- Uncaught `runPython` rejections auto-show a red overlay headlining the failing line of the .py file, with the full traceback one click away behind a `Traceback` toggle — good default for debugging; catch the rejection yourself when you want custom error UI.
