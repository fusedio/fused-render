# usd-core deliberately absent from `[bundled]`: convert_worker.py fetches it
# on demand (D119). numpy + msgpack come from the app's own interpreter, which
# the worker inherits too — reader.py spawns it detached with sys.executable.
