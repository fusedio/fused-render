...
# -- the INJECTED roots: what fused-render hands its own sessions -------------
# Not the user's config, which is what makes them worth reporting here at all
# (D491): all three sections of this page read `~/.claude`, and `--plugin-dir`
# is not in it, so a user hunting a missing skill found an empty page and no way
# to tell that from "the skill really is not being injected".


def _root(tmp_path, name, skills=("fused-render-ai",)):
    """A plugin root shaped the way the CLI's --plugin-dir loader wants."""
    root = tmp_path / name
    (root / ".claude-plugin").mkdir(parents=True)
    (root / ".claude-plugin" / "plugin.json").write_text('{"name": "%s"}' % name)
    for slug in skills:
        d = root / "skills" / slug
        d.mkdir(parents=True)
...
def test_plugins_injected_reports_the_roots_the_template_would_pass(
        client, claude_dir, tmp_path, monkeypatch):
    """Read off the ENV contract, not by recomputing `plugin_dir()` — that is
    the difference between "what a spawn passes" and "what we would like it to
    pass", and only the first can show a publication that failed."""
    from fused_render import skill_plugin

    monkeypatch.setenv(skill_plugin.PLUGIN_DIR_ENV,
                       str(_root(tmp_path, "skill-plugin")))
    monkeypatch.setenv(skill_plugin.WORKBENCH_PLUGIN_DIR_ENV,
                       str(_root(tmp_path, "workbench", ("canvas-toml",))))
    rows = _post(client, "plugins", action="injected").json()["plugins"]

    assert [r["name"] for r in rows] == ["fused-render", "workbench"]
    ours, workbench = rows
    assert ours["available"] is True
    assert [s["slug"] for s in ours["skills"]] == ["fused-render-ai"]
    # The frontmatter description is what answers "is the skill I need in
    # there", so it has to survive the trip.
    assert ours["skills"][0]["description"] == "Does fused-render-ai things."
    assert isinstance(ours["assembled"], int)
    # The workbench root is scoped to canvas targets by `_plugin_argv`, and the
    # row says so rather than implying it reaches every session.
    assert ours["scope"] == "every session"
    assert workbench["scope"] == "canvas sessions only"


def test_plugins_injected_reports_an_unpublished_root_rather_than_hiding_it(
        client, claude_dir, monkeypatch):
    """An unset var is the exact state the user was in, so it must come back as
    a row that says so — not as an omission indistinguishable from a page that
    failed to load."""
    from fused_render import skill_plugin

    monkeypatch.delenv(skill_plugin.PLUGIN_DIR_ENV, raising=False)
    monkeypatch.delenv(skill_plugin.WORKBENCH_PLUGIN_DIR_ENV, raising=False)
    rows = _post(client, "plugins", action="injected").json()["plugins"]
    assert [r["available"] for r in rows] == [False, False]
    assert [r["root"] for r in rows] == [None, None]
    assert rows[0]["env"] == skill_plugin.PLUGIN_DIR_ENV


def test_plugins_injected_ignores_a_root_that_is_not_a_plugin(
        client, claude_dir, tmp_path, monkeypatch):
    """A path with no manifest is not a plugin the CLI would load, so reporting
    it as available would promise skills no session receives."""
    from fused_render import skill_plugin

    bare = tmp_path / "bare"
    (bare / "skills" / "x").mkdir(parents=True)
    monkeypatch.setenv(skill_plugin.PLUGIN_DIR_ENV, str(bare))
    row = _post(client, "plugins", action="injected").json()["plugins"][0]
    assert row["available"] is False and row["skills"] == []


def test_plugins_rebuild_reassembles_and_returns_the_fresh_rows(
        client, claude_dir, tmp_path, monkeypatch):
    """The Rebuild button exists because the roots are assembled at server
    startup: a long-running server otherwise keeps handing out the tree it built
    at boot, which is how D490's missing skill survived an upgrade in place."""
    monkeypatch.setenv("FUSED_RENDER_HOME", str(tmp_path / "home"))
    body = _post(client, "plugins", action="rebuild").json()
    assert body["ok"] is True
    from fused_render import skill_plugin, skill_sources

    assert body["root"] == skill_plugin.plugin_dir()
    # The response carries the rows so the page needs no second round trip, and
    # they describe the tree that was just written.
    ours = body["plugins"][0]
    assert ours["available"] is True
    assert [s["slug"] for s in ours["skills"]] == list(skill_sources.skill_names())
    # …and the env var now points at it, which is what the next spawn reads.
    assert os.environ[skill_plugin.PLUGIN_DIR_ENV] == body["root"]
