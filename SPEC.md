...
- **AN-36** **Origin-view stamping:** every new thread carries `m` — the mode name of the view that created it (template folder name; `_render` for sentinel/standalone pages), derived from the overlay frame's own `/render` URL. Foreignness (AN-34) compares `m` against the active view **exactly**, so element comments from two different element-mode views never cross-resolve (a coincidentally shared element id would otherwise pin one comment in both views). The sidebar tag and switch-modes toast name the origin view. Threads without `m` (pre-AN-36) fall back to anchor-shape inference: selection ↔ editor, everything else ↔ element mode.
