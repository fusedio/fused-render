...
| `fused.capture.*` | ❌ | native capture (SPEC §44): the app's own process records the author's screen and microphone to the author's disk, behind a macOS permission granted to FusedRender.app. A hosted page has none of that, and the browser APIs it *does* have (`getDisplayMedia`, `MediaRecorder`) differ in every way that matters — picker per recording, no system audio, a blob rather than a path — so this is absent rather than shimmed. Like `fileIndex` it is **not** a blocking export error (the exporter's check requires `fused.<name>(`, so the dotted calls slip past), which makes it a `fused.env` gating obligation: ask `fused.capture` for `sources()` and hide the record button when `available` is false, or branch on `fused.env === "local"` before drawing one at all. |
