...
.html file (rendered in an iframe)
   ...
   │
   ├─ fused.ai.text({prompt})  ← the claude CLI, or a local model; {text, model, usage, provider}
   │
...
| `await fused.ai.text({prompt, ...opts})` | Ask an AI model; resolves with `{text, model, usage, provider}`. Local-only. See **"AI calls"** below. |
