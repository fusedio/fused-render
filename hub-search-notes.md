...
- **D-numbered entries added: D655, D656, D657** (in the restored
...
### Part D — this file, and D658/D659

Highest D-number was D633 on this branch (D630 on origin/main at the time
of resume, since renumbered to D655-D657 after a later merge collided with
origin/main's own unrelated D631-D633; see the note at the end of this
file) — D658 and D659 added, covering the measured-only quant rule and
...
packed 4-bit repos). Both are fixed this session — see D660/D661 in
...
`api_hub_search` already truncated to `limit` (D657's own row-positioning
...
for their own drops (see D656/D659) — `api_hub_search`'s own overfetch
...
## Second fix-builder round: GGUF rows still blank on this Mac (D662)
...
Decision recorded as **D662** in `DECISIONS.md` (appended, dated
...
decide format-filter-adjacent tests never written to exercise D662, the
...
Then all ten findings, MUST-FIX first. See D667-D674 in `DECISIONS.md` for
...
purpose until now). D667 has the rejected alternatives — in particular,
do NOT re-derive params from a mixed-repo's own (nulled) safetensors map;
D661 already settled that one row must describe ONE upload consistently.
...
itself, feeding the summary line, stays primaries-only per D664's own
...
`_popularity_score`'s 0.0 floor (D663's own deliberate exception).
...
- **"You already resolve a specific GGUF file for these rows, so its size
  ...
  size estimate to the wire response at all, on purpose (see D667's
...
## Merge-conflict note: D631-D633 collided with origin/main, renumbered to D655-D657
...
side; this branch's three were renumbered to **D655, D656, D657**
(preserving their original text and order, appended after this branch's
own D674) to clear `tests/test_doc_duplicate_ids.py`. Every in-repo
cross-reference to the branch's old D631/D632/D633 — in this file and in
`DECISIONS.md`'s own later entries (D660, D668) — was updated to the new
...
D675-D677 in DECISIONS.md carry the full reasoning; this is what changed and
what the brief got wrong.

**(1) GGUF params feed under-reported memory fit by ~3.4x for unfittable
models (HIGH, a cascade from D655's own fix).** `hub_models.py`'s
...
param — real as of D655) permanently beat the lazy lookup's REAL measured
...
on primaries only (D664's original call) — which reproduced the exact
...
model.fit == null`). A GGUF row whose `model.fit` is a non-null D655 GUESS,
...
alone (see D676's own "Rejected" column) rather than silently widening
scope.
...
D678-D681 in DECISIONS.md carry the full reasoning; this is what changed,
...
separate `fit_params` variable D675 introduced are deleted; there is exactly
...
implement "absence is neutral, not penal" (D663's own doctrine, re-verified
...
Kept the fix anyway (D681's own "Rejected" column explains why: unreachable
...
(D656) — `(row.get("fit") or {}).get("verdict")` reads `None`, not `"no"`,
...
- `.venv/bin/python -m pytest tests/test_ai_fit.py tests/test_ai_speed.py
  ...
  a GGUF row — didn't regress either module, and D678-D681 clear the
  duplicate-id guard).
...
D682-D683 in DECISIONS.md carry the full reasoning; this is what changed and
what this round's brief got wrong.

### (1) `_estimated_bytes` packed-dtype guard (D682)
...
three times (D658, D660, D678) for the exact same reason each time: a
whitelist/partial-fix approach keeps coming back in a new shape.

### (2) The hoist rule, unanimity-only (D683)
...
in the given set (primaries + variants, one set, per D677) agrees, and a
