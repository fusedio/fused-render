...
(D296), which is a different thing from the execution engine below however
...
def inference_engines() -> dict:
    """The per-capability INFERENCE engine preferences, as stored (D296).
...
def _prefs_response() -> dict:
    return {
        ...
        # Which local-model backend serves each capability (D296). The STORED
