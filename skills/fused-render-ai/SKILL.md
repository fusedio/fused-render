...
|---|---|
| **Diffusers (CUDA)** / **(ROCm)** / **(CPU)** — the default everywhere but Apple Silicon (CUDA or ROCm when a matching GPU is there, CPU as the fallback), and what a Mac switches to | the curated FLUX.2 klein repo, plus any ordinary diffusers repo (a `model_index.json`), via `AutoPipelineForText2Image`. The three read exactly the same repos — they differ only in which torch wheel is installed, so nothing about `model` changes between them |
...
**Memory is the failure mode to expect, and which memory depends on the engine.** On Apple Silicon, MLX FLUX reserves far more than Diffusers and is untested below 32GB. Off it, the default engine is now CUDA or ROCm when your machine has one — holding the pipeline in the **card's own VRAM** — and the CPU build only when neither is there, holding it in **system RAM** instead; so a machine that renders fine on its GPU can still OOM if it falls through to the CPU build, and vice versa on a GPU too small for the pipeline. Either way the OOM arrives as `ai_error`, so show `err.message` and name the Engines tab rather than working around it.
...
- **Minutes, not seconds on the CPU fallback — and capped at 900 s (15 min)** server-side, separately from the 600 s the text relay allows. **The default engine off Apple Silicon is CUDA or ROCm on a machine with a capable GPU, and the CPU build only on a machine with neither** — so the cap is mostly a CPU-fallback concern now, but it is still reachable on a GPU machine with a large enough canvas and step count. A 2048² 100-step render can hit it and comes back `ai_error` "the image process did not answer", so don't hand a slider the full clamp range.
...
- **Read `fused.ai.models.catalog()`'s `videoTraits.supportsImage` before offering this control — `/api/ai/video` does not check it for you.** There is exactly one video runner today (`ltx-video`, `supportsImage: true`), so this route has no per-engine gate on `image` — unlike `/api/ai/image`'s own `image`, which the server refuses outright for an engine that cannot edit. A future runner shipping `supportsImage: false` would accept an `image` you send it and silently ignore it rather than reject it, exactly the "accepted but not honoured" failure D413 exists to catch elsewhere — so read `supportsImage` and hide the control yourself; nothing on the server will do it for you.
...
MLX Embeddings on Apple Silicon, ONNX Embeddings everywhere else — DirectML,
CUDA or ROCm by default on a machine with the matching GPU, plain CPU as the
fallback where none is there. Unusually for this app, **the two engines produce vectors in the
...
|---|---|---|
| `text-generation` | MLX, then llama.cpp (Vulkan), then llama.cpp (CPU) | **Everywhere.** MLX on Apple Silicon; **llama.cpp (Vulkan)** on any Windows/Linux x86_64 machine `auto` resolves it on — vendor-agnostic (NVIDIA, AMD or Intel; the check cannot and does not distinguish vendor), and only a HINT of a usable device, not proof: on Linux it refuses when the only registered Vulkan driver is a known software rasterizer (lavapipe), but a machine with a real GPU driver registered alongside a broken one still passes; on Windows it checks that `vulkan-1.dll` exists AND that the Display class registry key lists a real adapter (refusing only when every entry is the Microsoft Basic Render Driver — a headless/RDP/VM session), but that still cannot tell a Vulkan-capable driver from a stale one behind a real GPU. A much larger download than the CPU build, fetched automatically the first time such a machine loads a model. **llama.cpp (CPU)** is the fallback for a machine that check refuses, or that has neither a GPU nor Apple Silicon (Metal comes through the same CPU-index wheel there, so it is on the GPU on a Mac too). **Local text ids are GGUF**: the curated ones are the GGUF's own filename, and any other GGUF repo resolves generically once picked from Hub search or loaded by its bare repo id. A plain safetensors repo is loadable only by MLX, i.e. only on Apple Silicon. A model too large for the Vulkan card degrades to partial or full CPU offload rather than failing the load. |
| `text-to-image` | MLX FLUX, then Diffusers (CUDA), then Diffusers (ROCm), then Diffusers (CPU) | **Everywhere.** MLX FLUX takes Apple Silicon (quicker, smaller download, much more memory); the CUDA and ROCm variants lead off Apple Silicon and are what `auto` fetches on a machine with a usable NVIDIA or AMD GPU — much larger downloads than the CPU build, chosen automatically rather than only from the Engines tab. Diffusers (CPU) is the fallback for a machine with neither, one Engines-tab switch away on a Mac — minutes per image rather than seconds. |
...
| `embeddings` | MLX Embeddings, then ONNX (DirectML), (CUDA), (ROCm), then ONNX Embeddings (CPU) | **Everywhere.** MLX takes Apple Silicon; the three accelerated ONNX rows lead off Apple Silicon and are what `auto` fetches on a machine with a matching GPU — DirectML answers yes on any Windows x86_64 box that has a Direct3D 12 adapter beyond the Microsoft Basic Render Driver (a headless/RDP/VM session with no real GPU shows only that one and is refused), vendor-agnostic the same way Vulkan is above. **ONNX Embeddings (CPU)** is the fallback for a machine with none, and is the Apple-Silicon fallback too, so **local embedding ids are ONNX exports** on every machine but a Mac (a parity gate asserts ≥0.999 cosine against the torch weights). |
