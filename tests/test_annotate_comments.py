...
persists forever. Unowned keys (claudeSessions/bookmarkHistory/lastSession) are
...
def test_record_creates_comments_key(tmp_path):
    ...
    assert resp == {"recorded": True, "count": 1}
...
def test_empty_array_is_no_op(tmp_path):
    ...
    f = _target(tmp_path)
    resp = ann._record(str(f), [])
    assert resp == {"recorded": True, "count": 0}
