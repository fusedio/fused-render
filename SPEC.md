...
- **PY-19** A project's `[project].requires-python` decides **which Python its environment is built on**. The pinned 3.12 (PY-18/D214) is the default and stays the answer for every declaration it satisfies — same interpreter, same venv key, no second CPython downloaded — but a declaration the default **fails** moves the base interpreter to the **nearest release the folder allows**, searched outward from the default so nothing moves further than the declaration demands (`==3.11.*` → 3.11, `>=3.13` → 3.13, `>3.13` → 3.13, `<3.11` → 3.10, `>=3.12.5` → 3.12). The floor is **3.10** — this app's own `requires-python`, hence the oldest interpreter the wrapper it embeds is known to parse on — and there is deliberately **no ceiling**: a pin above the default names its own version, so the newest honourable one is whatever uv can install, and a version uv has never heard of fails with uv's own words rather than against a constant somebody must remember to bump. That interpreter is acquired exactly like the pinned one — a `python` progress round under a **version-derived** bootstrap key, so two folders wanting two Pythons never share one download or one another's progress — and then handed to `uv sync --python`. The **app-interpreter fast path** (PY-17) is **declined** for a folder whose pin this app's own interpreter does not satisfy: skipping the venv is a claim that running here is equivalent to running there, and a version pin denies exactly that. A pin **nothing in range** can satisfy (`>=4.0`, `<3.9`, `==2.7.*`) is refused **before** any install, with its own error type naming the declaration and the range this app can offer — never a download that could not have helped, and never the generic engine error, which would blame fused-render for a line the user wrote. A pin that cannot be **parsed** is ignored with a warning rather than failing the run (uv rejects the same string with a better message when the environment is built). The comparison is **minor-granular**: what the app can obtain is `uv python install 3.X`, whose patch is uv's choice, so a **patch-level** pin (`>=3.12.5`, `<=3.12.9`) is answered on the 3.x line and, if the patch this machine has is the excluded one, surfaces as uv's own requires-python refusal naming both versions and the base interpreter — a loud error, never a run on a Python the folder ruled out. Editing `requires-python` is a manifest edit, so the digest moves and the environment is rebuilt on the newly-allowed interpreter (D244).
