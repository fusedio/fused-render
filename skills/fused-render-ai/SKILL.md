...
- **Only the MLX Whisper engine (Apple Silicon) produces them today.** CTranslate2 and Parakeet both *could* — neither is wired yet — so treat "no `words`" as normal, not as an error to report.
