"""runPython target for annotate-live/template.html: mirror the review's
comments into the target file's `<file>.json` sidecar as a write-only LOG.

This is a byte-for-byte copy of templates/annotate/annotate.py (record /
tombstone contract IDENTICAL) — annotate-live only adds a Liveblocks live layer
in template.html; the per-machine sidecar semantics are unchanged, so a remote-
authored comment lands in whichever machine's sidecar recorded it (documented,
acceptable). Keeping this file a parity copy lets its sidecar tests mirror
annotate's exactly.
