...
// SCORED — see `hubTableView.ts` for the cell rules and D780/D781/D782 in
...
// about to receive.
//
// **A family is one `<tbody>`, not one `<tr>` (D803).** The rows a disclosure opens
  ...
  return (
    ...
    >
      <TableRow
        ...
        style={arriving === null ? undefined : ({ "--am-part": `${arriving * 100}%` } as CSSProperties)}
      >
        {/* The merged Match cell (D780/D781): the printed number IS the
            composite `matchScore`, coloured by the memory verdict, and a
            non-GPU run mode (D782) prints as a visible muted suffix rather
        ...
        {/* Task/Capability, merged (D782): the value is `model.capability` —
...
export function HubResultsTable({
  ...
  // Hoisting (D781/D782): presence and the summary line must be computed
        ...
        <TableHeader>
          <TableRow>
            ...
            {/* Labelled "Capability", not "Task" — the cells beneath it
                render `model.capability` (D782), and a header must not
