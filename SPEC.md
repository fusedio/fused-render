...
- **PY-12** `/api/run` executes the built-in executor **by default**, regardless of whether the `fused` package is importable. `FUSED_RENDER_ENGINE=auto` opts in to running code through its local compute backend (`engine.py`) instead — fresh subprocess per call in a temp exec dir (PY-6 semantics preserved), PEP 723 `# /// script` inline requirements resolved into a cached venv (plus `engine.DEFAULT_REQUIREMENTS`, the shared set every core template's dependencies live in (PY-16)), params delivered via `_params.json` — falling back to the built-in executor if `fused` isn't importable; `FUSED_RENDER_ENGINE=fused` requires it (startup error if missing); `=builtin` (or unset) always uses the built-in executor (D70). The active engine is reported in `GET /api/config` (`engine`) and logged at startup — the choice changes the code contract, so it is never silent.
...
- **PY-16** A script's venv under the fused engine contains `engine.DEFAULT_REQUIREMENTS` plus that script's own PEP 723 header — **nothing else**, and in particular not the rest of the `[bundled]` extra (DM-2), which only the packaged app's interpreter ships. **Core templates carry no PEP 723 headers**: everything they need is in `DEFAULT_REQUIREMENTS`, which therefore tracks `[bundled]` closely (minus the server-only credential libs, plus the core `pyarrow`/`duckdb`). The venv cache is keyed on the sorted requirement set, so one shared set = one venv for all core templates, where per-template headers would mean one multi-minute build each. Headers remain the mechanism for **user** pages and for dependencies the app does not bundle. A header is read only on the file `/api/run` is handed: a helper module or a worker/daemon spawned with `sys.executable` runs in the *entrypoint's* venv, so declaring a dependency on the spawnee is inert. Enforced by `tests/test_engine_requirements.py` (which also parses every template's header, so a malformed block can't ship).
