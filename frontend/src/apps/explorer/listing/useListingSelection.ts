...
export function useListingSelection({
  ...
  // a row (selectionAfterVanish, D278). A link that misses is a link that missed.
  ...
  // row one (selectionAfterVanish, D278). So the -1 must reach that decision
  ...
  useEffect(() => {
    ...
    if (selectedPath === null) {
      ...
      // dropped it (D278), and the difference is what got us here: a pending
