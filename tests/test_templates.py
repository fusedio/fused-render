...
def test_claude_leads_the_universal_directory_key():
    # `claude` is the LEAD of the universal `/` key (D277) and `_listing` follows
...
def test_unmapped_file_empty_and_plain_dir_lists():
    ...
    # default for a folder, D277), the built-in listing, and the offered-but-gated
...
def test_plain_dir_without_markers_gates_false(tmp_path):
    ...
    # `_listing` moved to index 1 behind the gated `claude` (D277) and is STILL the
