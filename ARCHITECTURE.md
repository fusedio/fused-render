...
- `/view/<path>` → `stat` it:
  - a target with a non-empty `stat.templates` → preview view — **including a directory** (a `.zarr` store maps to a directory template, SPEC PT-13/D64) — unless the shell-owned `?listing=1` param forces the listing
...
TEMPLATES = {
    ...
    ".tif": ["geotiff"], ".tiff": ["geotiff"],            # in-browser decode, no reader.py (D63)
...
# Directory templates (SPEC PT-13/D64): a DIRECTORY whose basename carries one
