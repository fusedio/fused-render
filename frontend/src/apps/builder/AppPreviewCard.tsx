  ...
  return (
    ...
    >
      ...
      <span className="app-pcard-thumb" aria-hidden="true" ref={thumbRef}>
        ...
        {/* Hover-revealed export (SPEC §43 AF-4): the same action as the
            right-click menu's "Export App File", surfaced so it is one
            visible click. A <button> inside the card's <a>: it must both
            preventDefault (or the card link opens the app) and
            stopPropagation (or the click ALSO bubbles to onAppCardClick).
            Sits after the shield in DOM order so the shield can't eat it. */}
        <button
          type="button"
