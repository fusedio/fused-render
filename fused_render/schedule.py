...
from fused_render import claude_spawn, cron
...
def create(target: str, message: str, due=None, session_id: str = "",
           permission_mode: str = "", repeats: str = "") -> dict:
    ...
    if repeats:
        ...
        rule = cron.parse(repeats)
        when = _from_local(rule.next_after(_local_naive(_now())))
    else:
    ...
    entry = {
        ...
        "state": RECURRING if repeats else PENDING,
...
def _materialize(now: datetime) -> None:
    ...
    A template whose cron line no longer parses (a hand-edited store) is moved
    ...
    with _lock:
        ...
        for entry in entries:
            ...
            try:
                ...
                next_due = _from_local(rule.next_after(_local_naive(base)))
