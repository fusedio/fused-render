...
#
# **`gguf` (fix for code review finding 1, amending D661) is the same shape
...
# repo-wide-total bug D662's own fix corrected for `usedStorage`.
_EXPAND = (
...
# The DEFAULT ranking (D663): one 0-100 number blending memory fit, a
# params-based capability proxy, speed, recency and popularity, plus a small
# on-disk bonus — see `_composite_score` and D663 for the full defense of
...
# ---- D663's composite score: weights, defaults, and the axis curves -------
#
# Every weight below is a DELIBERATE choice, not a magic tuple — see D663 for
...
# A small nudge for a model already on this disk (D663) — it costs nothing to
...
# D665: a row that only runs via CPU offload or CPU-only is a real cost the
...
def _capability_score(params: int | None, ram_gb: float | None) -> float:
    """The params-as-capability-proxy axis (D663) — never a guess when
    `params` is unknown, which is exactly the row shape D658/D660 already
...
def _popularity_score(downloads: int | None) -> float:
    ...
    "nothing known" default is 0 rather than a middle value (D663): every
...
def _composite_score(row: dict, ram_gb: float | None) -> float:
    """`row["matchScore"]` (D663) — the composite 0-100 the DEFAULT sort
    ...
    * memory fit (`row["fit"]["score"]`, `fit.verdict`'s own 0-100) — the
      ...
      than tying with it the way the pre-D663 fit-only sort did.
    ...
    * popularity (`_popularity_score`) — log-scaled and the LOWEST weight, a
    ...
    row would not run on the GPU/unified memory (D665 — the speed axis is a
...
# quantized checkpoint (D658-amending finding, code review F2) — the same set
...
# — the thing D658 exists to rule out. `_params` must not sum these RAW either
...
def _estimated_bytes(safetensors) -> int | None:
    ...
    sum below is DOMINATED by a packed dtype (`_PACKED_DTYPES`, D658/D660's
...
def _quant(safetensors, file: str | None, config=None) -> str | None:
    ...
    * `config`'s own `quantization`/`quantization_config` block (D660,
      amending D658) — MLX writes `quantization: {bits}`, transformers writes
...
def _params(safetensors, config=None) -> int | None:
    ...
    quantized checkpoint's dtype map counts (D660, code review F2).
...
def _model_row(raw: dict, cache_dir: str, dirs: dict[str, str],
    ...
    **(D662) A GGUF pick is not limited to the ACTIVE runner.** D412 reads as
    ...
    **Why the fifth drop is allowed to depend on the resolved runner, when
    ...
    if runner is not None and "gguf" in runner.hub_filter_tags:
        ...
        # D662: the active runner speaks no format this picker knows, but
        ...
        # runner's alone to make (see the docstring's D662 paragraph).
    ...
    #
    # **Read for EVERY GGUF repo, not only a `file`-resolved one** (D676).
    ...
    #
    # `params_from_gguf` joins `file` as a reason to refuse (D676), and it has
    ...
    # (D676) The repo's weight FORMAT, when the Hub said something that
...
def api_hub_search(body: dict = Body(default={}), x_fused: str | None = Header(default=None)):
    ...
    **A POST, and guarded, even though it reads nothing on this machine.** Every
    ...
    # D663: the composite match score, not raw downloads, is the default —
    ...
    # D663: every row gets a `matchScore` regardless of which sort was asked
