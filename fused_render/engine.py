...
def _error_dict(err_type: str, message: str, tb: str = "") -> dict:
    ...
    # where=None: none of these callers (missing file, unreadable, bad PEP 723
    # header, backend/engine internal failure) ever reached the user's file.
    return {
        "ok": False,
...
def _user_location(cleaned: str, script_path: str) -> dict | None:
    ...
    Text-based counterpart of _child.py's `_user_location`: the fused engine
    only has the backend's formatted string (execution ran in a subprocess),
    not a live exception object. Works for a runtime frame and for a
    SyntaxError's displayed block alike — build_code compiles the user's file
    as its own unit under its real filename, so a SyntaxError's `File
    "<script_path>", line N` block (no ", in <func>") matches `_FRAME_LINE`
    the same way a normal frame does. Keeps the LAST matching frame — the
    deepest one, matching `_child.py`'s "last wins" semantics. `source` is the
    line directly under the frame header, when the traceback printed one
    (skipped for a bare caret/tilde annotation line or no line at all).
    """
    location = None
