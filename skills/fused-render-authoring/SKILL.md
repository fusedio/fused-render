...
| `await fused.navigate(route, params?, config?)` | Navigate the shell to another route of the **enclosing fused_app** — the nearest ancestor directory of this page's file holding a valid `fused_app.json` (resolved server-side). `route` uses the manifest `pages[].path` spelling (`"/about"`; `"/"` = the entry page, clean URL). `params` (string values) **merge** onto the shell URL's current params by default; `config: {params: "overwrite"}` replaces them instead — reserved `_`-prefixed shell params survive both modes. Resolves with `{app_dir, url}`; rejects with a clear error when no enclosing app exists. |
