...
// (`app_entry`). While the pane still had a row MODE that could resolve to
// `claude` (a selected folder's default view), making that mode's default
// without taking the chat's own pane away would have put the app page back on
// screen — nested one level deeper, running the same Python, for the same mere
// selection. `chatOnly` is what removes it, and the chat checks it BEFORE it
// looks an entry page up at all. D460 deleted that row mode, but the `claude`
// COMPANION is the same chat in the same column, so the flag is still
// required.
describe("paneChatOnly", () => {
  test("the chat never gets a pane of its own in here", () => {
...
describe("the pane never previews a folder as its app page", () => {
  ...
  test("the pane's chat-only rule still guards its one claude surface", () => {
    ...
    // selected row's own templates at all), so the `claude` COMPANION — now the
    // app's own chat rather than a framed template — is the only caller left:
    // one call, asked once, handed to the mount as a prop. And no query
    // literal at all any more, because the chat has no URL to carry one.
