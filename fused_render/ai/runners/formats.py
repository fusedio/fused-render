...
#: a question about the FILES, which did not change when D413 removed the torch
...
#: wrong-format refusal, and it pointed the user at this engine. Since D413
...
#:
#: **This was a dict of refusal SENTENCES until D413, and the sentences went
...
#: re-measurement before anyone reversed the refusal. D413 is that
...
#: never implies a capability. (It was "the two text runners" until D413 removed
...
def loaders(*, repo_id: str, names, dirnames, config: dict, torch_weights: bool,
    ...
    if is_parakeet_checkpoint(config) and PARAKEET_WEIGHTS in names:
        ...
        return tuple(found)
    # A directory of safetensors, which since D413 exactly one engine here
