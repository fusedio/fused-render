      ...
      <main className={"cc-main" + (tab === "playground" ? " pg-fill" : "")}>
        <div className="cc-page-head">
          <div>
            ...
            {/* Monospace only for the cache PATH below — the Usage and Engines
                captions are plain sentences, and the Engines tab's own note
                (`.am-engines-note`) sits right under this one in the same
                proportional font. Applying `.cc-mono` to every branch made
                those sentences look like filesystem facts next to the one
                that is. */}
            {/* No caption on the PLAYGROUND, where the tab below it is the
                ...
                keeps its caption — they carry a fact the tab does not repeat (a
                window, a cache path, which backend runs what). */}
            {tab !== "playground" && (
