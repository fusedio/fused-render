// Rubber-band selection: press on the listing's empty space, drag, and every
// row the rectangle touches becomes selected. Pure geometry — the pointer
// wiring is useMarquee.ts.
//
// The gesture is split from the row drag by WHERE THE PRESS LANDS, not by any
// arbitration afterwards: a press on a row starts a move-drag (the browser's
// own drag-and-drop, see drag-drop.ts), a press on the background starts a
// marquee, and neither can turn into the other mid-gesture. A press that never
// travels MARQUEE_DRAG_SLOP is neither — it is the click the listing already
// had, and it must keep meaning select-this-row / open-this-row.
//
...
// The rectangle two points describe, whichever way round they are. A drag up
// and to the left has to name the same box as the same drag down and right, or
// it would have negative extent and intersect nothing.
...
// Which rows the box selects, given what was already selected.
//
//   • by default the marquee REPLACES the selection — the gesture starts on
//     empty space, which reads as "start again from here";
