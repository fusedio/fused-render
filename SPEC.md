...
| `.jsonl .ndjson` | `code`, `duckdb`, `claude`, `versions`, `reader` | append-only record streams. They carry the authored-file pair like any other text key: PT-14's question is "is this authored", not "does this diff well". No `git`, for the reason no file key has it (GT-2): the working tree is the folder's, not the file's |
...
`git` is offered on FOLDERS ONLY, and `versions` on files and folders alike**
...
- **GT-2** **Registry binding — the universal `/` directory key, and NO file key.
  `git` is a FOLDER-ONLY mode.** It sits immediately after `versions` in
  `"/": ["_listing", "app", "claude", "versions", "git", "graph", "zarr_aoi"]`
  and appears in none of the ~47 **authored-file** keys, which keep `versions`
  ...
  per-file question — what happened to this file — is `versions`', which is
  ...
  write or analyse these bytes" is unchanged as the question for `versions` and
  ...
  *What this is, historically.* This is the D235 binding restored, with a
  ...
  a file's commit story was `versions`' and a file offering both would be two
  ...
  `versions` holds — not because a file wanted a working tree, but because a
...
`versions`, the repository's history of the file, and this panel, the agent's
