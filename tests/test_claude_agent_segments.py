...
def test_history_reports_the_latest_assistant_usage_as_context(
    ...
    each request re-sends the whole conversation, so the newest row's three
    input numbers already are the whole prompt that went up the wire — adding
    the older rows in would double-count the same tokens every turn."""
    ...
    # 3 + 20 + 2000 — the three INPUT fields of the last reply, summed; the
    # output is reported beside them and deliberately not added in (it only
    # counts against the window on the next request, as input).
    assert out["context"] == {"tokens": 2023, "output_tokens": 9,
                              "model": "claude-opus-5[1m]"}
...
def test_history_context_ignores_junk_usage_without_losing_the_payload(
    ...
    assert out["context"] == {"tokens": 500, "output_tokens": 2,
    ...
    assert out["context"] == {"tokens": 12, "output_tokens": 0, "model": ""}
