...
def _make_repo(root):
    """A repo-shaped tree. Its `.gitignore` is inert content, not a rule the
    walk reads — the floor is name-based only (DEFAULT_IGNORE_NAMES prunes
    `dist` on its own), so this fixture exists for the tests below that need a
    repo present, not for the ignore patterns it happens to write."""
...
def test_walk_non_repo_tree_unaffected(tmp_path):
    ...
    assert "dist/bundle.js" in rels  # the walk's floor is WALK_IGNORE_DIRS,
    # not the index's larger DEFAULT_IGNORE_NAMES — "dist" isn't in it
    assert "debug.log" in rels


def test_walk_stream_prunes_default_ignored_too(tmp_path):
    repo = tmp_path / "repo"
    _make_repo(repo)
    client = _client(tmp_path)
    lines = _stream_lines(client, str(tmp_path))
