...
| `embeddings` | MLX Embeddings, then ONNX (DirectML), (CUDA), (ROCm), then ONNX Embeddings (CPU) | **Everywhere.** MLX takes Apple Silicon; the three accelerated ONNX rows lead off Apple Silicon and are what `auto` fetches on a machine with a matching GPU (DirectML answers yes on any Windows x86_64 box, since it runs on any Direct3D 12 adapter). **ONNX Embeddings (CPU)** is the fallback for a machine with none, and is the Apple-Silicon fallback too, so **local embedding ids are ONNX exports** on every machine but a Mac (a parity gate asserts ≥0.999 cosine against the torch weights). |
