...
beside `worker_base.py`, `formats.py` and `preview.py`, for the reason
`preview.py` states about itself: THREE folders serve this one engine —
`transformers_text/`, `transformers_text_cuda/` and `transformers_text_rocm/` —
and they differ only in which index their `pyproject.toml` takes torch from. CPU
is the default, CUDA and ROCm are variants a user picks on the Engines tab, and
the hardware they select is a fact about the wheel, never about the code. Each of
those folders holds a `worker.py` that is a five-line shell around
`torch_text.main()`; a copy of this file under any of them would fail no test,
because each copy would pass its own.
