...
def test_syntax_error_where_from_exception(tmp_path):
    ...
    # format_exception_only renders the file/line/caret block, which points at
    # the user's file — but never the "Traceback (most recent call last):" head.
    assert "Traceback (most recent call last):" not in out["error"]["traceback"]


def test_harness_error_has_no_where_and_no_stack(tmp_path):
    # A missing required param is raised by the binder (bind_params), never in
    # the user's file: where is None and the traceback is the exception line
    # only — no stack, no runner frames.
    path = _write(tmp_path, "t.py", "def main(count: int):\n    return count\n")
    out = run_python(path, {})
    assert out["ok"] is False
    assert out["error"]["type"] == "ParamError"
    assert out["error"]["where"] is None
    tb = out["error"]["traceback"].strip()
    assert "Traceback (most recent call last):" not in tb
    assert tb.endswith("missing required param: 'count'")
