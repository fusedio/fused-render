...
| Paths root | `~/.fused-render` (flat, shared with dev/CLI; cache under `%LOCALAPPDATA%\FusedRender\cache`) | `~/Library/Application Support/fused-render` | `~/.fused-render` (flat, shared with dev/CLI; cache under `$XDG_CACHE_HOME/fused-render`, runtime under `$XDG_RUNTIME_DIR`) |
