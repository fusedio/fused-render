...
def _win_protected(entry: "os.DirEntry") -> bool:
    ...
    try:
        ...
        return False
    return bool(attrs & _srv.stat_mod.FILE_ATTRIBUTE_HIDDEN
                and attrs & _srv.stat_mod.FILE_ATTRIBUTE_SYSTEM)
...
def _list_direct(path, cursor):
    ...
    listing = pathops.list_mount_dir(
        path, cursor=cursor, max_entries=_srv.S3_LIST_MAX_ENTRIES,
        overall_timeout=_srv.S3_LIST_OVERALL_TIMEOUT_S, allow_rc_fallback=False)
...
def _walk_bfs(path, include_hidden, max_entries=None, max_depth=None):
    ...
    def oracle_for(repo):
        ...
        if oracle is None:
            ...
            while len(oracles) >= _srv.WALK_MAX_ORACLES:
    ...
    try:
        ...
        while queue:
            ...
            if mount_backed:
                ...
                try:
                    listing = pathops.list_mount_dir(
                        current, max_entries=_srv.WALK_MAX_ENTRIES_REMOTE,
                        page_timeout=_srv.WALK_RC_LIST_TIMEOUT_S,
                        overall_timeout=_srv.WALK_RC_LIST_TIMEOUT_S,
                        rc_timeout=_srv.WALK_RC_LIST_TIMEOUT_S)
                ...
                if listing.direct:
                    ...
                    if len(listed) > _srv.WALK_MAX_ENTRIES_REMOTE:
                        dir_cut = True
                        listed = listed[:_srv.WALK_MAX_ENTRIES_REMOTE]
                if dir_cut:
                    yield _WALK_TRUNCATED
                children = [
            ...
            for child in children:
                ...
                try:
                    ...
                    continue
                if is_dir:
                    if name in _srv.WALK_IGNORE_DIRS:
                        continue
                    dirs.append(child)
                else:
                    files.append(child)
            ...
            for child, is_dir in [(d, True) for d in dirs] + [(f, False) for f in files]:
                ...
                if is_dir:
                    ...
                    if not is_link and not child.name.lower().endswith(_srv.WALK_LEAF_DIR_SUFFIXES):
                        if not can_descend:
                            depth_capped = True
...
def _mount_list_error_response(path, exc):
    ...
    if isinstance(exc, shell_mounts.RcListUnavailable):
        ...
        return _srv._error(broken or f"cannot list directory {path}", status=503)
    broken = shell_mounts.broken_mount_error(path)
    if broken:
        return _srv._error(broken, status=503)
    return _srv._error(f"not a directory: {path}", status=400)
