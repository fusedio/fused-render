...
class _WatchEntry:
    ...
    def __init__(self, path: str):
        ...
        if self.is_mount:
            ...
            self.interval = (_srv._MOUNT_POLL_S if self._direct_capable
                             else _srv._MOUNT_SLOW_POLL_S)
        else:
            self._direct_capable = False
            self._is_mount_root = False
            self.interval = _srv._LOCAL_POLL_S
        self.subscribers: set = set()  # asyncio.Queue per socket
        self.last = _srv._UNCHANGED  # primed by the first successful read
    ...
    async def _read(self):
        ...
        if self._inflight is not None:
            if not self._inflight.done():
                return _srv._UNCHANGED  # previous read still hanging; skip this tick
        ...
        try:
            sig = await asyncio.wait_for(
                asyncio.shield(self._inflight), _srv._STAT_TIMEOUT_S)
        except asyncio.TimeoutError:
            return _srv._UNCHANGED  # leave _inflight running; consumed on a later tick
        self._inflight = None
        return sig
    ...
    async def run(self):
        ...
        while True:
            ...
            if sig is not _srv._UNCHANGED:
                if self.last is not _srv._UNCHANGED and sig != self.last:
                    self._broadcast(sig)
                self.last = sig
