...
def test_a_removed_engine_code_in_prefs_degrades_to_the_ordering(monkeypatch):
    """The three codes D413 deleted, arriving in a prefs.json that outlived them.
...
#: catalog, notes and docs describe — which is exactly what D413 did do, and it
...
def test_every_shipped_platform_keeps_a_local_text_engine(monkeypatch):
    ...
    read about and not use. D413 is why it is now written as an INVARIANT over an
...
def test_intel_macos_is_not_advertised_as_a_supported_text_platform(monkeypatch):
    ...
    it too (as a distribution decision rather than a packaging one), so D413
...
def test_apple_silicon_falls_back_to_llamacpp_when_mlx_is_unavailable(
    ...
    an MPS-capable macOS wheel. Since D413 it is `llamacpp-text`, whose CPU-index
...
def test_cuda_is_a_HARD_GATE_on_a_machine_with_no_nvidia_gpu(monkeypatch, tmp_path):
    ...
    # D413 the CUDA row under test is an image row — text generation's own
...
def test_AUTO_STAYS_ON_THE_UNACCELERATED_ROW_EVEN_WITH_AN_ACCELERATOR(
    ...
    **Renamed from `test_AUTO_STAYS_ON_THE_CPU_ROW_EVEN_WITH_AN_ACCELERATOR` at
    D413, and the text-generation half of it went with the transformers rows.**
...
def test_the_cpu_rows_name_the_apple_silicon_GPU_they_run_on(monkeypatch):
    ...
    for (D382); it went at D413, and the property outlived it because it was
    ...
    for code, cap in (("llamacpp-text", 170), ("diffusers-image", 110)):
        ...
        # rather than PyPI — and since D413 made this the default engine on two
...
def test_llamacpp_and_whisper_suggestions_show_snapshot_size_estimates():
    ...
    It read the `transformers-text` list until D413 — the bf16 Qwen entries at
    ...
    what they replaced is the measurement D413 rests on, sitting here as data.
    """
    expected = {
...
def test_llamacpp_text_is_registered_directly_below_mlx_text(monkeypatch):
    ...
    **This test asserted the opposite until D413 and the inversion is the
    ...
    thing that made "never a fallthrough" achievable; D413 weighed that against
...
def test_llamacpp_text_vulkan_is_registered_immediately_below_llamacpp_text(monkeypatch):
    ...
    generation: D413 gave `llamacpp-text` the default and deliberately did not
...
def test_llamacpp_text_vulkan_is_reachable_only_by_an_explicit_preference(monkeypatch, tmp_path):
    """Opt-in, UNLIKE its neighbour since D413: a working loader and ICD make
...
def safetensors_text_engine(monkeypatch):
    ...
    read safetensors and served Windows and Linux — and D413 removed it, leaving
...
def test_a_text_model_the_CHOSEN_engine_cannot_open_is_not_offered(
    ...
    The engine switched TO was `transformers-text` until D413; `llamacpp-text` is
...
def test_a_runner_with_no_suggestions_offers_the_disk_and_recommends_nothing(
    ...
    **The no-suggestions condition is CONSTRUCTED, and the construction is asserted.**
    ...
    # D413 is a different format per platform: safetensors for `mlx-text`, a
