...
- **`width`/`height` default from the base image, not from 1024².** The image is fit to a longest side of 1024 without upscaling, snapped down to a multiple of 16, floored at 256, aspect preserved — read the reply rather than assuming your own file's dimensions survive unchanged. An explicit `width`/`height` still wins.
...
- **No seed-reproducibility claim.** mflux is not byte-identical across two renders with the same seed on the version this app ships — a fact about the engine, unrelated to whether you passed `image` — so do not build a "regenerate identically" feature on `seed` for this call.
