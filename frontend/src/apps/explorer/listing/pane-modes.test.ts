...
// PT-13): **`claude` first** (D278) with `_listing` behind it, and every peer
...
describe("paneModeList — the selected target", () => {
  test("a selected subfolder defaults to CLAUDE, the registry's lead", () => {
    // D278: the pane reads the list for its lead (`activePaneMode` with no
  ...
  test("a folder whose claude gate says NO falls back to the listing", () => {
    // The deliberate answer to "what if the default is denied" (D278): the next
  ...
  test("a folder holding a lone page is still just a folder", () => {
    ...
    // (D269, deleted by D278). A folder's modes are the FOLDER's, and the page it
...
// (D278, deleting D269's pane half). The pane used to resolve a folder with a
