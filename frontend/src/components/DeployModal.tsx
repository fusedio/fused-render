...
import type { DeployConfig, Deployment, ShareMount } from "../lib/api";
...
// The env's mounts (`fused share list`), joined server-side to the local pages
// that deployed them — the "which of my files is deployed" view. Loaded
// lazily: a share list shells the CLI (and may hit the network), so it only
// runs when the section is expanded.
function SharesSection({ env, fsPath }: { env: string; fsPath: string }) {
  ...
  return (
    <div className="deploy-shares">
      <button type="button" className="deploy-shares-toggle" onClick={() => setOpen(!open)}>
        {open ? "▾" : "▸"} Shares on {env}
      </button>
      {open && (
        <div className="deploy-shares-body">
          ...
          {mounts && mounts.length === 0 && (
            <div className="deploy-muted">No shares on this environment.</div>
          )}
          {mounts && mounts.length > 0 && (
                ...
                {mounts.map((m) => {
                  ...
                  return (
                    <tr key={m.token} className={mine ? "mine" : undefined}>
                      <td className="share-page" title={m.page ?? undefined}>
                        {m.page ? basename(m.page) + (mine ? " (this page)" : "") : "—"}
                      </td>
                      ...
                      <td>
                        {m.url ? (
                          ...
                          <span className="deploy-muted">no URL</span>
                        )}
                      </td>
                    </tr>
                  );
