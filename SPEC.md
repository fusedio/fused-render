...
- **FS-9** **THE PANE IS NOT A CHOICE — it is a property of how much room the listing container has.** Above `PANE_SPLIT_MIN_W` (700 px, measured on the container with a `ResizeObserver`, never read off `window.innerWidth`) the listing splits; below it the listing has the container to itself and is exactly the listing FS-1..FS-8 describes. Measured on the **container** because the same `Listing` renders full-window, inside a chrome-free embed, and inside another view's split, and only the container knows which. 700 px sits well above the 280 px both floors technically fit in (FS-12): the question was never "can it be laid out" but "is a half-width listing beside a half-width preview still worth reading", and at 700 px both halves are ~350 px — a listing that still shows a name and a size, and a preview big enough to recognise what it is showing. An unmeasured container (0, `NaN` — an unattached or `display:none` element) reads as **no**, which is the safe answer: painting a pane on a guess and tearing it away on the first real measurement is the one visible failure here. *This replaces a **Default OFF** clause and everything that hung off it. The pane used to be opt-in per folder, and that one bit lived in three places that had to agree — a toggle button, a sticky `?preview=true` URL param carried across directory navigation, and a `pane=0` view-state key so a folder remembered being closed. Three writers for one bit, and the bit was almost always a proxy for a question the layout can answer itself.*
- **FS-10** **NO TOGGLE — there is nothing to toggle.** `shouldShowPane(containerW)` is the whole of the on/off decision (FS-9): no button, no URL param, no saved on/off state, so the split is right on a wide window, right in a narrow embed, and right the moment either is resized, with nothing to restore and nothing to go stale. There is still no separate mode, no `_mode` value and no registry entry for "listing with a pane" — the pane is a property of *how this folder is being viewed*, not a different view of it (precisely the distinction the `preview` template got wrong). The pane's **header strip** survives the toggle's removal and is present in every pane state (loading skeleton, error, metadata card, multi-selection placeholder, self target), but it now carries only the previewed row's identity and its own controls; nothing in it closes the pane. *This clause used to specify the toggle as **one affordance in two places, chosen by state**: an icon-only "Hide preview" as the first item of the pane's header strip (on the seam it collapses toward), and a labelled glyph + "Preview" button in the listing's search row for reopening, the two never coexisting. Both are deleted, along with the drag-to-close gesture that let the divider shut the pane at the right edge — **closing needs a way back**, and once the split is decided by width there is no reopen affordance to offer, so a pane dragged shut would have stayed shut until the window was resized. The drag now just holds at the pane's floor, which is what the clamp already did all the way to the edge.*
