...
- **FS-16** **Opening a folder auto-selects its first file**, so the pane opens showing something. With the pane on and nothing else claiming the selection, the listing selects the **first non-directory row in the rendered order** (the active sort's order — the row the eye is already on) the first time the folder's listing settles. Otherwise the pane opened on the folder's own **self target** (FS-11), whose default state is the `Select a file to preview.` hint: a pane that opens empty asks the user to do the obvious thing before it will do anything at all, on a view they opened precisely to see what is inside. **Directories are skipped, not previewed** — a folder's preview is a peek, and landing on one would make merely opening a folder feel as though it had navigated somewhere. A folder with **no files at all** (empty, or only directories) keeps the self target and its hint, unchanged.

  **It never fights an existing claim, and it is ONE SHOT PER FOLDER NAVIGATION** (the listing component remounts per path). The claim it defers to is the **`?sel` URL param**, and that param is the *only* reading of the selection it takes: the seeding effect applies `?sel` in the same commit, so the component's own selection state is still the pre-seed value there — and a **bare** URL means the selection is about to be empty whatever that state holds, since the seed *clears* a selection recalled across a remount when the URL carries none, and after the seed the lead is mirrored back into `?sel`. (Reading the state instead was a real bug: browsing into a file and back to its folder arrived with a recalled selection that was being discarded on that very commit, which burned the shot and left the pane empty for the whole mount.)

  Three conditions **hold** the shot rather than spending it, because each can still turn into a folder the user is looking at: the pane being **off** (toggling it on later still lands on the first file), **search** mode (the rendered rows are a query's answer, not the folder's, so clearing the query still lands), and a listing that has not settled. An **embedded** listing (the pane's own `_listing` mode) never fires at all — it has no pane to fill. Because it is one shot, a dir-watch refresh never re-fires it and a deliberately cleared selection stays cleared — the auto-selection is an opening move, not a rule that the selection must be non-empty.
