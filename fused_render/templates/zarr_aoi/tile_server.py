...
def _serve():
    ...
    class MeterStore(WrapperStore):
        ...
        async def _store_get(self, key, prototype, byte_range):
                    ...
                    if fs is not None and root:
                        ...
                        if size > 16 * 1024 * 1024:
                            ...
                            parts = await asyncio.gather(
                                *(
                                    fs._cat_file(
                                        full, start=i * step, end=min(size, (i + 1) * step)
                                    )
                                    for i in range(n)
                                )
                            )
            ...
            try:
                ...
                if isinstance(byte_range, SuffixByteRequest) and root and "http" in proto:
                    n = int(byte_range.suffix)
                    session = await fs.set_session()
                    async with session.get(f"{root}/{key}", headers={"Range": f"bytes=-{n}"}) as r:
                        if r.status == 404:
                            return None
                        r.raise_for_status()
                        data = await r.read()
    ...
    def resolve_source(path):
        ...
        if path.startswith(("http://", "https://")):
            return {"kind": "http", "url": path, "storage_options": {}, "label": path}
        ...
        if path.startswith(mroot):
            ...
            if os.environ.get("ZARRAOI_TRANSPORT", "raw") != "s3":
                ...
                origin = os.environ.get("FUSED_RENDER_ORIGIN", "http://127.0.0.1:1777")
                return {
                    "kind": "http",
                    "url": origin + "/api/fs/raw?path=" + quote(path, safe="/"),
                    "storage_options": {},
            ...
            try:
                mounts = json.load(open(os.path.join(_home_dir(), "mounts.json")))
            except (OSError, ValueError):
                mounts = []
            ent = next((m for m in mounts if m.get("name") == name), None)
            if ent and ":" in ent.get("remote", ""):
                rname, _, rpath = ent["remote"].partition(":")
                key = "/".join(s for s in (rpath.strip("/"), rest.replace(os.sep, "/")) if s)
                cfg = rclone_conf().get(rname, {})
                if cfg.get("type") == "s3":
                    so = dict(S3_POOL)
                    anon = cfg.get("env_auth", "false") != "true" and not cfg.get("access_key_id")
                    if anon:
                        so["anon"] = True
                    ck = {}
                    if cfg.get("region"):
                    ...
                    return {
                        ...
                        "url": "s3://" + key,
                        "storage_options": so,
                        "label": f"mount '{name}' → s3://{key}" + (" (anonymous)" if anon else ""),
                    }
                    ...
                    return {
                        ...
                        "label": f"mount '{name}' → gcs://{key}" + (" (anonymous)" if anon else ""),
                    }
        ...
        if md.get("zarr_format") == 3:
            ...
            for c in md.get("codecs", []):
                if c.get("name") == "sharding_indexed":
                    ...
                    info["codecs"] += [
                        cc.get("name") for cc in cfg.get("codecs", []) if cc.get("name") != "bytes"
                    ]
    ...
    def open_dataset(path):
        ...
        if src["kind"] == "local":
            ...
            inner = zarr.storage.FsspecStore.from_url(
                src["url"], read_only=True, storage_options=src["storage_options"]
            )
        ...
        if isinstance(ms, dict) and ms.get("layout"):
            for ent in ms["layout"]:
                ...
                if tr is None and levels and levels[0]["transform"]:
                    ...
                    tr = [t0[0] * sc[1] / s0[1], 0.0, t0[2], 0.0, t0[4] * sc[0] / s0[0], t0[5]]
                levels.append(
                    {
                        "asset": str(ent["asset"]),
            ...
            default_var = (
                max(
                    var_names,
                    key=lambda n: (
                        g0_arrs[n].dtype.kind == "f",
                        g0_arrs[n].ndim,
                        int(np.prod(g0_arrs[n].shape)),
                    ),
                )
                if var_names
                else None
            ...
            var_names = [
                ...
                if a.ndim >= 2 and not k.lower().endswith(("_bnds", "_bounds", "bnds"))
            ]
            if var_names:
                ...
                var_names = [n for n in var_names if dims_of(arrs[n])[-2:] == dims[-2:]]
                H, W = a.shape[-2], a.shape[-1]
                tr = None
                ydim, xdim = dims[-2] if len(dims) >= 2 else None, dims[-1] if dims else None
                try:
                    ...
                    tr = None
                levels.append({"asset": "", "shape": [H, W], "transform": tr, "scale": [1.0, 1.0]})
    ...
    def read_window(ds, level_i, var, index, r0, r1, c0, c1):
        ...
        pre = tuple(min(max(int(index), 0), a.shape[k] - 1) if k == 0 else 0 for k in range(nd - 2))
    ...
    def merc_to_lonlat(mx, my):
        return (
            np.degrees(np.asarray(mx) / MERC_R),
    ...
    def pick_level(ds, deg_per_px):
        cands = [
            (i, abs(lv["transform"][0])) for i, lv in enumerate(ds["levels"]) if lv["transform"]
        ]
    ...
    def sample_stats(ds, var, index):
        ...
        data, _ = metered(
            ds,
            "stats-sample",
            lambda: read_window(ds, coarse, var, index, r0, r1, c0, c1),
            level=coarse,
            window=[r1 - r0, c1 - c0],
        )
    ...
    def do_tile(q, z, x, y):
        ...
        px_lons = norm_lon(lv, np.linspace(lon0, lon1, TILE + 1)[:-1] + deg_per_px / 2)
        r0, r1, c0, c1 = window_of(lv, float(px_lons.min()), lat0, float(px_lons.max()), lat1)
        if r1 <= r0 or c1 <= c0:
        ...
        data, _ = metered(
            ds,
            "tile",
            lambda: read_window(ds, li, var, index, r0, r1, c0, c1),
            z=z,
            level=li,
            window=[r1 - r0, c1 - c0],
        )
        ...
        yin = (iy >= 0) & (iy < r1 - r0)
        xin = (ix >= 0) & (ix < c1 - c0)
        out = data[np.ix_(np.clip(iy, 0, r1 - r0 - 1), np.clip(ix, 0, c1 - c0 - 1))].astype(
            "float64"
        )
        out[~yin, :] = np.nan
        out[:, ~xin] = np.nan
        return (
            200,
            encode_png(np.ascontiguousarray(colorize(q, ds, var, index, out, fill))),
            "image/png",
        )
    ...
    def do_meta(q):
        ...
        logical = sum(int(np.prod(lv["shape"])) * extra_cells * itemsize for lv in ds["levels"])
        extra = []
        if a0.ndim > 2:
            dims = dims_of(a0)
            extra = [
                {"name": dims[k] if k < len(dims) else f"dim{k}", "size": int(a0.shape[k])}
                for k in range(a0.ndim - 2)
            ]
        out = {
            "file": ds["path"],
            "source": ds["src"]["label"],
            "kind": ds["src"]["kind"],
            "geographic": ds["geographic"],
            "vars": ds["vars"],
            "selected": var,
            "index": index,
            "extra_dims": extra,
            ...
            "chunks": ci["chunks"],
            "inner_chunks": ci["inner"],
            "chunk_logical_bytes": int(np.prod(ci["inner"] or ci["chunks"] or [0])) * itemsize,
            "codecs": ci["codecs"],
            "zarr_format": a0.metadata.to_dict().get("zarr_format"),
            ...
            "stats": s,
            "attrs": {str(k): str(v)[:400] for k, v in ds["attrs"].items() if k != "multiscales"},
        }
    ...
    def do_probe(q):
        ...
        if not (0 <= row < H and 0 <= col < W):
            return 200, json.dumps({"value": None, "inside": False}).encode(), "application/json"
        data, ent = metered(
            ds,
            "probe",
            lambda: read_window(ds, 0, var, index, row, row + 1, col, col + 1),
        ...
        out = {
            ...
            "inner_chunk": ([row // inner[-2], col // inner[-1]] if inner else None),
            "cost": {
    ...
    def do_hist(q):
        ...
        r0, r1, c0, c1 = window_of(lv, float(ln.min()), lat0, float(ln.max()), lat1, pad=0)
        if r1 <= r0 or c1 <= c0:
            return 200, json.dumps({"count": 0}).encode(), "application/json"
        if (r1 - r0) * (c1 - c0) > WINDOW_CAP:
            return 200, json.dumps({"count": 0, "skipped": True}).encode(), "application/json"
        data, _ = metered(
            ds,
            "hist",
            lambda: read_window(ds, li, var, index, r0, r1, c0, c1),
    ...
    def do_prefetch(q):
        ...
        def work():
            t0 = time.perf_counter()
            try:
                read_window(ds, 0, var, nxt, H // 2, H // 2 + 1, W // 2, W // 2 + 1)
                with ds["meter"]["lock"]:
                    ds["meter"]["ops"].appendleft(
                        {
                            "t": time.time(),
        ...
        return 200, json.dumps({"ok": True, "next_index": nxt}).encode(), "application/json"
