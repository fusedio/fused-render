...
- **PT-10** **Mode switcher (shell, preview header):** rendered only when `templates.length > 1`, right side of the preview header bar. Its contents are exactly the resolved list — the **available width is never an input** to it, so a mode cannot appear or vanish as a divider is dragged (PT-15). **Icon-only buttons**, mode name via native `title` tooltip, active mode in accent color. When an entry's `icon` is `null`, the shell renders a placeholder: the first letter of the mode name in a small rounded box. The `.html` Rendered|Source pair is **not a special case**: it is the ordinary mode list `["_render", "code"]` (PT-12) riding this same switcher — `_render` gets a shell-baked eye icon (sentinels have no folder to ship `icon.svg`); `code` gets its real folder icon. The `_listing` sentinel likewise gets a shell-baked list icon (D81). **No two modes offered on the same key may read the same LABEL.** A mode's display name is its folder name capitalized unless `MODE_TITLES` overrides it (a folder name is chosen for the filesystem, not for a tooltip), and that fallback is what makes collisions easy to create from a distance: `versions` was titled "History" while `history` reached the identical string through the fallback, and `.parquet` and `.html` each carry both — so the Open With menu (`lib/fs-actions.ts`, `label: modeTitle(t.mode)`) and this switcher's tooltips drew two indistinguishable entries for two different templates. Dispatch keys on `mode`, so it is a labelling defect only, which is exactly why nothing caught it. The names now say what each shows: **`versions` is "Revisions"** (a git commit timeline for one path) and **`history` is "Activity"** (the `<file>.json` sidecar's sessions, bookmarks and comments, §24). The same rule covers the preview pane's own list (FS-12/`listing/pane-modes.ts`), where the pane-only `_app` sentinel and the `app` template are both offered for a workspace app folder: `_app` is **"Folder app"**, the folder's lone top-level page rendered in place. `ModeSwitcher.test.ts` derives every co-offered set from the shipped registry and fails on any duplicate label, so the guard is the rule and not the pair.
