...
- **AI-2** **A runner is a folder, and its environment is `envinstall`'s.** Each
  ...
  `runners/llama_text.py`. (D413 removed a third such family — `transformers_text`
...
- **AI-2a** **A runner declares WHEELS, and uv's children do not inherit this
  ...
  **Amended (D411): the rule is WHEELS, not PyPI.** AI-11's original refusal of
  ...
  for PyTorch's CPU index (D413) and `diffusers_image_cuda`'s still does, so the
...
- **AI-11** **Text generation runs on every supported desktop platform, on the
  ...
  **Superseded in part (D413): the `transformers_text` runner and its two
...
- **AI-11d** **Reasoning is OFF by default, because it is invisible and the CPU
  ...
  unreferenced variable, so no retry is needed there at all. D413 removed the
...
- **AI-11b** **The device is reported, because a model on a CPU works and looks
  ...
  `gpu` there, and D413 made that row the DEFAULT on two more platforms, which
...
- **AI-11c** **No text has ever been generated by this runner, and AI-10b's
  ...
  stubs. (Written of `runners/torch_text.py`; D413 removed that module and
...
- **AI-11f** **`llamacpp-text` is a FOURTH text runner, GGUF via
  ...
  `auto` never reaches it on any platform** (D411). **AMENDED (D413): those three
  ...
  **What does NOT survive checking is the assumption that a wheel existing
  ...
  D413; the pointer it added is now unnecessary, since llama.cpp is the engine a
  ...
  D413, LAST — so it is the one text row `auto` still never reaches, sharing
...
- **AI-11h** **The `transformers-text` family is WITHDRAWN — all three rows, all
  ...
  else, with `llamacpp-text-vulkan` the one opt-in row below them** (D413).
  ...
  **Two consequences that are costs, accepted rather than argued away.**
  ...
  (reported separately, PR #706). That is why D413 moved the default onto
