...
# ---- which model the pills open on (Akshil, 2026-09-18) ----------------------
#
# "What I select as a user stays." `_defaults` answered a question about a
# FOLDER — the model last used anywhere in it — and every door into a chat asked
# it that way, so the same conversation opened from the Tasks peek, from that
# peek's Open button, from a row, from the chat list or from a bare URL could
...
def _ran(model, effort, cwd):
    """An assistant row as Claude Code writes one — which is where the model a
...
def test_the_named_session_answers_for_itself(agent, target):
    """The whole fix. Two conversations in one folder, and the newer one is not
    the one being asked about."""
    file, workdir = target
    _cli_transcript(agent, workdir, "older",
                    [_ran("claude-opus-4-6-20260514", "max", workdir)], mtime=500)
...
def test_a_pill_the_reader_changed_mid_CHAT_is_what_comes_back(agent, target):
    """The reported sentence, from the other end: a model picked in the composer
