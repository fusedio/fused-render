...
/* NO ENGINE HUE ANY MORE (2026-08-25). `ENGINE_HUES`, `engineHue` and
   `engineHueStyle` lived here for one job: giving each engine FAMILY one of the
   eight categorical `--task-c*` hues, so the tag carrying a card's most
   load-bearing fact could be picked out by colour while SWEEPING a grid (D436).

   There is no such tag left to sweep. Which backend reads a model's weights is
   identity, not state, so it moved into the (i) panel on every card that had it
   — and inside a panel a reader has deliberately opened, about one model, a hue
   is decoration: nothing is being told apart from anything. The panel states the
   engine in the same grey as every other fact in it.

   Deleted rather than left unused: the assignments were the point ("the one lime
   hue is spent deliberately, on the rarest family"), and a table nobody reads is
   a decision nobody is making. The `--task-c*` palette itself is untouched and
   still belongs to the calendar. */
