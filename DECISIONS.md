...
| # | Decision | Choice | Rationale / rejected alternatives |
|---|---|---|---|
| D55 | Execution engine | **openfused** (`fused>=2.9` on PyPI) `LocalPythonComputeBackend` replaces `executor.py`/`_child.py` — new module `fused_render/engine.py`. Fresh subprocess per call retained (D5's isolation mechanism intact), 30 s timeout kept, result caching disabled (`cache_storage=None`). `requires-python` floor moves to **3.11** (tomllib for PEP 723 parsing; openfused's own floor). Dev installs use the local sandbox checkout editable (`pip install -e ~/Documents/fused/sandbox/openfused`); packaged builds pull `fused` from PyPI | One engine shared with the fused platform instead of a bespoke runner; the backend interface gives a path to remote execution (Lambda/hosted) with no fused-render changes. Rejected keeping the hand-rolled executor alongside (two contracts to document and test) |
