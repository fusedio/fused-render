...
**Split preview pane (SPEC FS-9..FS-15, D185):** `Listing.tsx` optionally splits into list + preview pane. The toggle is **split by state** (SPEC FS-10): the search row's labelled "Preview" button renders only while the pane is CLOSED, and the open pane carries its own icon-only collapse control as the **first** item of `ListingPreviewPane`'s header strip (the pane's left edge is the seam it collapses toward) (`onCollapse`, which is `Listing`'s `togglePane` passed down — an embedded Listing never renders a pane at all, so there is no nesting case). That strip is rendered by every one of the pane's states, including the headerless ones it used to skip, since while the pane is open it is the only carrier of the close affordance. The pane's file case builds the **same** iframe src as `TemplatePreview` (`/render?path=<default template>&_file=<file>`, SPEC PT-2/PT-8) — one URL construction, so a file previews identically in the pane and full-screen — including PT-9's tail: it picks the first unconditional entry, and for an **all-conditional** list resolves the gates (`resolveConditions`) and shows the first allowed one rather than falling through to the metadata card, matching `Preview.defaultTemplate`'s `templates[0]` fallback; the same PT-12 sentinel filter runs first so a dangling `path: null` entry can never build a `path=null` render URL — and unlike full-screen `Preview`, which mounts `<Listing>` in place of the iframe for the `_listing` sentinel, the pane has no slot for a nested `Listing`, so `_listing` entries are excluded from the pane's candidates entirely (skipped in favor of the file's next template, or the metadata card if `_listing` was the only one). The directory case is a peek (single-app embed, else a read-only mini child list), not a nested `<Listing>`. **The split is stored as a FRACTION of the container** (SPEC FS-12): pane state is `{ on, frac, sized }`, `frac` defaults to `PANE_DEFAULT_FRAC = 0.5`, and `Listing` renders it as `flexBasis: ${frac * 100}%` — so a window resize rescales the pane instead of stranding it at one window's arithmetic, and nothing has to be measured before the first paint (the old measuring `useLayoutEffect` and its `PANE_FALLBACK_W = 420` are gone). The pixel floors live in the drag and in CSS: `dragPaneFrac(containerW, rawPx)` (with `clampPaneWidth` and `parsePaneFrac`, all three in the router-free `listing/pane-math.ts` so they are testable with no DOM) runs the cursor's distance from the container's right edge through the shared `clampPaneWidth(containerW, width)` (pane ≥ `PANE_MIN_W` 220 px, list ≥ `LIST_MIN_W` 60 px, `PANE_MIN_W` applied last so a container too small for both keeps the pane's floor and scrolls the list) and divides the clamped pixels back out into a fraction, returning `null` for a container too narrow to hold both floors (< 280 px, where the clamp is degenerate and any fraction would describe the container rather than a choice) so that such a drag moves nothing and persists nothing; `.listing-pane-slot` and `.listing-main` carry the same two floors as `min-width`, which is what holds them under a *window* resize. `sized` is provenance rather than geometry — true when `frac` was restored from `panew` or set by a drag that actually moved the divider — and the two `savePaneState` callers (toggle, divider pointer-up) pass `null` unless it is set, so a plain toggle cannot write the default into `panew` as though it had been chosen. It rides in state (not a ref) so every `setPane` updater can read it, and resets with the component, which remounts per `fsPath`, matching `panew`'s per-folder scope. `parsePaneFrac` rejects any `panew` value `> 1` as a **legacy pixel width** — ignored as if absent rather than translated, since the pixels were measured against a container this window may not have. `savePaneState` writes the two keys **independently** — `pane` follows the toggle, `panew` is only rewritten (or cleared when unsized) — so toggling the pane off does not delete a dragged split that the next open would otherwise have lost. **Visibility and width persist differently** (SPEC FS-13): the split fraction is viewstate-only (`panew` in the per-path store that already holds the folder's sort), so one machine's drag never enters a bookmark (SPEC SB-2); visibility rides the **URL** as a sticky `?preview=true` *and* viewstate (`pane=1`), with `resolvePane()` treating a present `?preview` as authoritative and an effect reflecting a viewstate-restored pane back into the URL so the two cannot disagree. `navigate()` in `lib/router.ts` carries the param onto **directory** targets only — which is now the whole of it, since `Breadcrumb.tsx`'s crumbs are plain `navigate(target, { isDir: true })` calls that drop `_mode` so a top-bar hop always lands on the listing (SPEC FS-13); **file** targets drop it deliberately, since `preview` is unreserved and runtime.js's D72 ancestor-climb would expose it to the template iframe a file view always hosts. Residual, documented in FS-13: a template rendered *inside the pane* does see it, because the listing URL legitimately carries it then. Loading states are skeleton shimmer, and the pane is never on the listing's critical path — a stalled pane leaves the list interactive. **Opening a folder auto-selects its first entry** (SPEC FS-16, D240): one effect in `Listing.tsx` owns the TIMING — armed per mount, consumed at the first settled non-search listing **with the pane on** — and `autoSelectPath(navRows, rowCtxByPath)` owns the decision (`firstEntryPath`, the first row — file or directory — in RENDERED order; a directory previews as the pane's peek, not a navigation). The decision takes **no** reading of the selection and none of the URL: the `?sel` param it used to defer to has been removed (it rewrote the address bar on every arrow-key press for a shareable link to a highlighted row). The **effect** does read it, once — `selectionClaimed(sel)` — because auto-select fills an empty selection and never replaces one: a row clicked in the provisional scaffold rides across the swap in the recall store, and row one used to land on top of it. The shot is spent either way. One shot, so a dir-watch refresh cannot re-fire it and a cleared selection stays cleared; but the pane being off (or search being active) HOLDS the shot rather than spending it, so toggling the pane on later still lands on the first file. **The pane also owns the plain row click** (SPEC FS-15): `onRowClick` navigates only when `!pane.on`, and `onRowDoubleClick` navigates only when `pane.on` — so exactly one of the two handlers opens a row, chosen by pane state, and neither branch needs a click-delay timer (the first click of a double-click selects, and the navigation supersedes the pane fetch it started). Shift/Mod clicks and `Enter` are outside this switch and behave identically with the pane on or off. This replaces the deleted `templates/preview/` folder, which was a vanilla-JS fork of this component (D185); `preview` is gone from the built-in `"/"` list, now `["_listing", "git", "graph", "zarr_aoi"]` (D193 added `git`, SPEC §33).
