    ...
    try:
        ...
        if is_mount_backed(path):
            return _srv._error(
                ...
                status=503)
        return _srv._error(f"no such file or directory: {path}", status=404)
    return _srv._stat_payload(path, stat_mod.S_ISDIR(st.st_mode), st)
...
def _fs_write(body: dict, x_fused: str | None):
    ...
    if not isinstance(content, str):
        return _srv._error("'content' must be a string")
    ...
    if shell_mounts.is_mount_backed(path):
        ...
        try:
            pr = _srv._mount_probe(path)
        except (shell_mounts.RcListUnavailable, shell_mounts.RcListTimeout) as e:
            return _srv._mount_list_error_response(parent, e)  # indeterminate -> 503
        if pr.exists and pr.is_dir:
            return _srv._error(f"path is a directory: {path}")
        if not pr.parent_is_dir:
            return _srv._error(f"parent directory does not exist: {parent}", status=404)
        ...
        try:
            ...
            try:
                ...
                pass
            return _srv._error(f"cannot write {path}: {e}", status=400)
        ...
        return _srv._mount_stat_payload(path, False, size, mtime)
    ...
    return _srv._stat_payload(path, False)
...
def _fs_mkdir(body: dict, x_fused: str | None):
    ...
    if shell_mounts.is_mount_backed(path):
        ...
        try:
            ...
            return _srv._mount_list_error_response(parent, e)  # indeterminate -> 503
        if not pr.parent_is_dir:
            return _srv._error(f"parent directory does not exist: {parent}")
        if pr.exists:
            return JSONResponse({"error": "conflict"}, status_code=409)
        try:
            os.mkdir(path)  # through the rclone VFS
        except OSError as e:
            return _srv._error(f"cannot create directory {path}: {e}")
        return _srv._mount_stat_payload(path, True, None, None)
    ...
    try:
        ...
        return _srv._error(f"cannot create directory {path}: {e}")
    return _srv._stat_payload(path, True)
...
def _fs_delete(body: dict, x_fused: str | None):
    ...
    if shell_mounts.is_mount_backed(path):
        ...
        try:
            ...
            return _srv._mount_list_error_response(os.path.dirname(path), e)  # 503
        if not pr.exists:
            return _srv._error(f"no such file or directory: {path}", status=404)
        if pr.is_dir:
            return _srv._error(
        ...
        try:
            ...
            return _srv._error(f"cannot delete {path}: {e}")
    ...
    if trash:
        ...
        if not _srv._trash_supported():
            return JSONResponse({"error": "trash unsupported"}, status_code=501)
        try:
            _srv._move_to_trash(path)
            ...
            return _srv._error(f"cannot move to Trash: {e}", status=500)
...
def _fs_rename(body: dict, x_fused: str | None):
    ...
    if not src or not os.path.isabs(src):
        return _srv._error("'src' must be an absolute filesystem path")
    if not dst or not os.path.isabs(dst):
        return _srv._error("'dst' must be an absolute filesystem path")
    ...
    if shell_mounts.is_mount_backed(src) or shell_mounts.is_mount_backed(dst):
        if shell_mounts.mount_read_only(src) or shell_mounts.mount_read_only(dst):
            return JSONResponse({"error": "readonly"}, status_code=403)
        try:
            src_pr = _srv._probe_path(src)
            dst_pr = _srv._probe_path(dst)
        except (shell_mounts.RcListUnavailable, shell_mounts.RcListTimeout) as e:
            return _srv._mount_list_error_response(
                os.path.dirname(src) if shell_mounts.is_mount_backed(src)
                else dst_parent, e)
        if not src_pr.exists:
            return _srv._error(f"no such file or directory: {src}", status=404)
        ...
        if not dst_pr.parent_is_dir:
            return _srv._error(f"parent directory does not exist: {dst_parent}")
        ...
        if not shell_mounts.is_mount_backed(src) and not _srv._writable(src):
            return JSONResponse({"error": "readonly"}, status_code=403)
        if not shell_mounts.is_mount_backed(dst) and not _srv._writable(dst):
            return JSONResponse({"error": "readonly"}, status_code=403)
        try:
            if dst_pr.exists:
            ...
            return _srv._error(f"cannot rename {src} -> {dst}: {e}")
        return _srv._mutation_result_payload(dst, False)
    ...
    if os.path.isdir(src):
        ...
        if d == s or d.startswith(s + os.sep):
            return _srv._error("cannot move a directory into itself or a descendant")
    ...
    if not _srv._writable(src) or not _srv._writable(dst):
        return JSONResponse({"error": "readonly"}, status_code=403)
    ...
    try:
        ...
        return _srv._error(f"cannot rename {src} -> {dst}: {e}")
    return _srv._stat_payload(dst, os.path.isdir(dst))
...
def _fs_copy(body: dict, x_fused: str | None):
    ...
    if shell_mounts.is_mount_backed(src) or shell_mounts.is_mount_backed(dst):
        ...
        try:
            ...
            return _srv._error(f"cannot copy {src} -> {dst}: {e}")
        return _srv._mutation_result_payload(dst, False)
    ...
    if src_is_dir:
        ...
        d = os.path.abspath(dst)
        if d == s or d.startswith(s + os.sep):
            return _srv._error("cannot copy a directory into itself or a descendant")

    dst_exists = os.path.exists(dst)
    if dst_exists and not overwrite:
        return JSONResponse({"error": "conflict"}, status_code=409)
    if not _srv._writable(dst):
        return JSONResponse({"error": "readonly"}, status_code=403)
