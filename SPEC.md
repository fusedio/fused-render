...
- **FS-11** **Selection-driven content.** The pane renders whatever the currently selected row is:
  ...
  - **nothing selected** → the **self target**: the pane targets the folder *already open on the left*, resolves its modes exactly as above, and **drops `_listing`** (that listing *is* the left half) — but it **lands on NO mode at all**. "Nothing previewed" is a **state, not a mode**: `activePaneMode` returns `null` for a self target and the pane renders its neutral empty-state hint (`Select a file to preview.`, the same `.pane-hint` the empty pane already had) under the ordinary header. The one exception is a lone-app folder's `_app` — the folder's own app, a preview of the folder rather than an opt-in tool — which still leads the list *and* still defaults. This is a default, not a restriction: the switcher (PT-10) still lists every offered mode, with **none of them marked active**, so the chat (`claude`) or `git` is **one click away**; there is simply no way to select "no preview" *back*, because closing the pane is that gesture. The reason is that the `/` key's peers are heavyweight opt-ins (D235/D237 put the chat on every folder), so with `_listing` dropped the first mode is `claude`: a plain first-wins default meant merely toggling the pane on opened an AI prompt box as a folder's default preview, which is not what "preview this folder" asks for. A self target with **nothing** offerable at all (every peer gate-denied, no app) gets an **empty** mode list and shows the bare hint with no header — there is no mode to switch to. An explicit choice still wins and still persists through `_panelMode` like any other pane mode; a stale `_panelMode` naming a mode this target doesn't offer falls back to the default per PT-9, which for a self target is the neutral state. The pane never shows a stale previous preview. *There used to be a pane-only `_none` ("No preview") entry carrying this instead of a null default — overturned by the owner: it put "No preview" in the mode menu as something to pick, which it never was.*
- **FS-12** **Draggable divider** between list and pane. **The split is a FRACTION of the split container, not a pixel width** — state is `{ on, frac }`, rendered as a percentage `flex-basis`, defaulting to `PANE_DEFAULT_FRAC = 0.5` (half and half). A proportion is what the user actually chose ("about a third of the window to the list"), and it is the only form of the answer that stays right when the window resizes: the previous model resolved a measured half into pixels at mount and then never rescaled, so a pane sized on a wide monitor became the whole view on a laptop. A percentage also needs **no measurement** — it is correct before the first paint, whatever the container turns out to be — so the measuring layout effect and its unmeasurable-container fallback (`PANE_FALLBACK_W = 420`) are both gone. **The pixel floors remain, and they are where the pixels live now:** the drag runs the cursor's distance from the container's right edge through `clampPaneWidth` (pane ≥ 220 px, list ≥ 60 px, the pane's floor applied last so a container too small for both keeps the pane and scrolls the list) and stores `clamped ÷ container width`; `.listing-pane-slot` and `.listing-main` carry the same two floors as CSS `min-width`, which is what holds them when the *window* — not the divider — is what changed. Only a **dragged** fraction persists (FS-13); the default is never written to `panew`.
...
- **FS-16** **Opening a folder auto-selects its first file**, so the pane opens showing something. With the pane on and nothing else claiming the selection, the listing selects the **first non-directory row in the rendered order** (the active sort's order — the row the eye is already on) the first time the folder's listing settles. Otherwise the pane opened on the folder's own **self target** (FS-11), whose default state is the `Select a file to preview.` hint: a pane that opens empty asks the user to do the obvious thing before it will do anything at all, on a view they opened precisely to see what is inside. **Directories are skipped, not previewed** — a folder's preview is a peek, and landing on one would make merely opening a folder feel as though it had navigated somewhere. A folder with **no files at all** (empty, or only directories) keeps the self target and its hint, unchanged.
  ...
  **It never fights an existing claim, and it is ONE SHOT PER FOLDER NAVIGATION** (the listing component remounts per path): a `?sel` seed wins (read from the URL, since the seed lands in the same commit), as does a selection carried across the pre-stat provisional→resolved remount or any row the user has already clicked; it does not run in **search** mode (where the rendered rows are a query's answer, not the folder's), nor in an **embedded** listing (the pane's own `_listing` mode — no pane to fill), nor while the pane is **off**. Because it is one shot, a dir-watch refresh never re-fires it and a deliberately cleared selection (Escape, a background click) stays cleared — the auto-selection is an opening move, not a rule that the selection must be non-empty.
