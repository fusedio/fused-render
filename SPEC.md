...
- **PY-16** A script's venv under the fused engine contains **exactly that script's own PEP 723 header and nothing else** — no baseline is unioned in, so a header is the script's complete dependency list in the PEP 723 sense, not a delta against an invisible set. In particular it does not contain the rest of the `[bundled]` extra (DM-2), which only the app's own interpreter ships (PY-17). A worker or daemon spawned with `sys.executable` inherits the spawning script's environment, so its dependencies must be declared on the **spawner** — a header on a non-entrypoint file is never read. A core template may carry a header **only if** all three hold: it is **read** (the file is a runPython entry point — a header on a helper module or a spawned daemon is inert), it is **necessary** (it declares something outside `[bundled]` + the core `dependencies`), and it is **complete** (it covers every such distribution the file, or anything it can execute, imports). A template that manages its own venv for a daemon declares its dependencies there, not in a header — that is the only form the built-in engine can honor too (D174). All three are enforced by `tests/test_engine_requirements.py`, which derives entry points and invocation relationships from the source (`_runpython_targets`/`_module_refs`/`_venv_roots`) rather than a maintained list, and parses every template's header so a malformed block can't ship (D172).
- **PY-17** A script with **no** PEP 723 header runs on **the app's own interpreter** and gets no venv at all: the app ships `[bundled]` + its core `dependencies`, so numpy/pandas/duckdb/rasterio/… are available with no download and no first-run wait. The interpreter is **verified, not assumed** — it is run once per server process and must report this app's own `sys.prefix`, probed under the child's stripped environment (the backend removes PYTHONHOME/PYTHONPATH, which a packaged interpreter may need to locate its stdlib). An autodetected candidate whose basename is not python-shaped is rejected without being spawned. If the direct candidate fails, the app builds a **bootstrap venv** off it (`python -m venv --system-site-packages --without-pip`, plus a `.pth` carrying the server's own `sys.path`) and verifies THAT instead — accepted only when the app's package directories are on the child's `sys.path`. This is the packaged macOS path, not an edge case: the bundled interpreter locates its runtime only via `PYTHONHOME`, which the backend strips. If no interpreter can be verified, a header-less script **fails with a configuration error** naming `FUSED_RENDER_APP_PYTHON` — it is never silently degraded to a venv, because with no baseline requirements that venv has no data stack and would fail on the first import, and because a header-less core template must never reach the network. Nothing in this resolution installs anything: `--without-pip` makes that true by construction. `FUSED_RENDER_APP_PYTHON` overrides the candidate (still probed) (D172, D175).
