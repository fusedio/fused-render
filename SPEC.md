...
- **FS-11** **Selection-driven content.** The pane renders whatever the currently selected row is:
  ...
  - **nothing selected** → the **self target**: the pane targets the folder *already open on the left*, resolves its modes exactly as above, and **drops `_listing`** (that listing *is* the left half) — but it **lands on a neutral placeholder, never on a mode**. The self target's list therefore carries the pane-only **`_none`** sentinel ("No preview", the empty-state hint) ahead of every *template* mode, and only a lone-app folder's `_app` — the folder's own app, a preview of the folder rather than an opt-in tool — still outranks it. This is a default, not a restriction: the switcher (PT-10) still shows every one of them, so the chat (`claude`), `git` or a lone-app `_app` is **one click away and one click back** — and because `_none` is a real entry the switcher always has ≥2 entries to render, which is what keeps them reachable. The reason is that the `/` key's peers are heavyweight opt-ins (D230/D232 put the chat on every folder), so with `_listing` dropped the first *template* mode was `claude`: merely toggling the pane on opened an AI prompt box as a folder's default preview, which is not what "preview this folder" asks for. A self target with **nothing** offerable but `_none` shows the bare hint with no header at all — there is no mode to switch to. `_none` is pane-local like `_app` (not a registry sentinel, never sent to the server) and is selectable/persisted through `_panelMode` like any other pane mode; a stale `_panelMode` naming a mode this target doesn't offer falls back to the default per PT-9, so a `_panelMode=_none` carried onto a *selected* row lands on that row's own default (`_listing` for a folder). The pane never shows a stale previous preview.
...
- **PY-12** `/api/run` executes the built-in executor **by default**, regardless of whether the `fused` package is importable. `FUSED_RENDER_ENGINE=auto` opts in to running code through its local compute backend (`engine.py`) instead — fresh subprocess per call in a temp exec dir (PY-6 semantics preserved), a script with no PEP 723 header running on the app's own interpreter (PY-17) and one with a header getting a cached venv built from exactly what it declares (PY-16), params delivered via `_params.json` — falling back to the built-in executor if `fused` isn't importable; `FUSED_RENDER_ENGINE=fused` requires it (startup error if missing); `=builtin` (or unset) always uses the built-in executor (D70). The active engine is reported in `GET /api/config` (`engine`) and logged at startup — the choice changes the code contract, so it is never silent.
...
- **PY-16** A script's venv under the fused engine contains **exactly that script's own PEP 723 header and nothing else** — no baseline is unioned in, so a header is the script's complete dependency list in the PEP 723 sense, not a delta against an invisible set. In particular it does not contain the rest of the `[bundled]` extra (DM-2), which only the app's own interpreter ships (PY-17). A worker or daemon spawned with `sys.executable` inherits the spawning script's environment, so its dependencies must be declared on the **spawner** — a header on a non-entrypoint file is never read. A core template may carry a header **only if** all three hold: it is **read** (the file is a runPython entry point — a header on a helper module or a spawned daemon is inert), it is **necessary** (it declares something the platform's app interpreter genuinely lacks — judged against the macOS bundle's real contents, not `[bundled]`'s promises, and scoped with a PEP 508 environment marker when only one platform needs it — D176), and it is **complete** (it covers every such distribution the file, or anything it can execute, imports). A template that manages its own venv for a daemon declares its dependencies there, not in a header — that is the only form the built-in engine can honor too (D174). All three are enforced by `tests/test_engine_requirements.py`, which derives entry points and invocation relationships from the source (`_runpython_targets`/`_module_refs`/`_venv_roots`) rather than a maintained list, and parses every template's header so a malformed block can't ship (D172).
- **PY-17** A script with **no** PEP 723 header runs on **the app's own interpreter** and gets no venv at all: the app ships `[bundled]` + its core `dependencies`, so numpy/pandas/duckdb/rasterio/… are available with no download and no first-run wait. The interpreter is **verified, not assumed** — it is run once per server process and must report this app's own `sys.prefix`, probed under the child's stripped environment (the backend removes PYTHONHOME/PYTHONPATH, which a packaged interpreter may need to locate its stdlib). An autodetected candidate whose basename is not python-shaped is rejected without being spawned. If the direct candidate fails, the app generates a **wrapper script** that restores the `PYTHONHOME` this process depends on and `exec`s the real interpreter, then verifies THAT the same way. This is the packaged-macOS path, not an edge case: measured on a real DMG, the bundled interpreter stripped of `PYTHONHOME` reports the *build machine's* Homebrew framework as its prefix, and the bundle ships no `venv` module, so a venv-based rescue is impossible there. The wrapper sets the child's `sys.executable` to itself (`exec -a`), so a daemon re-spawned as `[sys.executable, …]` — geotiff, zarr_aoi, usd — keeps working even though those templates scrub `PYTHONHOME` from the environments they spawn into. Wrappers are POSIX-only and generated **only** when this process actually needs `PYTHONHOME`; Windows and the Linux AppImage self-locate and stay on the direct candidate. If no interpreter can be verified, a header-less script **fails with a configuration error** naming `FUSED_RENDER_APP_PYTHON` — it is never silently degraded to a venv, because with no baseline requirements that venv has no data stack and would fail on the first import, and because a header-less core template must never reach the network. Nothing in this resolution installs anything. `FUSED_RENDER_APP_PYTHON` overrides the candidate (still probed) (D172, D175).
- **PY-18** A script whose header names something not installed yet gets an **explicit install flow**, never a blocking download inside `/api/run`: the endpoint answers `needs_install` (venv key + resolved requirements, alongside a normal `error` object), `POST /api/env/install` spawns a detached worker that writes `{stage, pct, detail, done, error, pid, ts}` to `progress.json`, `GET /api/env/progress?key=` polls it, and `POST /api/env/cancel` stops it by the recorded pid. `runtime.js` shows the loader and retries the run **once**, so every template gets this without its own code. Installer failures reach the user **verbatim** — a resolver's own message ("no matching distribution / no wheels with a matching platform tag") is the answer, never a generic engine error. Progress is deliberately coarse (the underlying install captures its output, so per-package progress is unavailable) and reports only stages it can observe. Scope is **per-file**: the venv belongs to one .py's requirement set; sharing one across a folder is out of scope (D173).
...
- **PT-16** **The chat template's contract: one gate, three pane shapes, and a system prompt that cannot disagree with the pane (D232).** `templates/claude/` is the single chat mode (PT-14). Because it is bound to two kinds of target it branches on kind in exactly two places — the left pane and the prompt — and both read the **same** predicate, `shared/app_entry.entry_html`, so what the prompt claims is beside the chat is what is beside the chat.
  ...
  - **The left pane, three shapes.** A **file** → the file in its OWN default template: `GET /api/fs/stat` for the target, drop `conditional` entries (their verdict lives behind `/api/fs/conditions` and is deliberately not fetched — an unresolved gate reads as "not offered") and drop the chat mode itself (a pane framing the chat again is a mirror, not a preview), then frame `/render?path=<that template>&_file=<file>` — or the file itself when the entry is the `_render` sentinel. That is the shell's own `defaultTemplate` rule (PT-8) reused rather than a per-extension table inside the chat, which would drift from the registry on the next rebinding and ignore a user override (§16); and it is a **default, not a lock** — the pane-local `leftmode` param (PT-15) selects any other offerable entry from that same stat payload, unknown values falling back silently as in PT-9. An **app folder** (an entry page resolves) → that entry page, via `/render`. A folder with **no** app entry → **`/explorer/embed/<dir>`**, the chrome-free navigable shell (LM-4/D39): a real file browser beside the chat, so the folder's contents are answerable by looking and the user can walk into a file mid-conversation. That embed is opened as **`?preview=false&modechip=false`**, and both params exist for the same reason: a directory's counterpart mode *is* this chat (PT-14), so the embed's own preview pane and the embed's corner mode chip (`.preview-browse-chip`, pinned top-right in `body.embed`) each offered a **second chat — its own agent, its own composer — nested inside the first one's pane**. `modechip=false` is read straight off `location.search` in `Preview.tsx`, the `preview=false` param's shape (literal `false`; absent = chip on, so every other embed keeps it), and it is also PT-15(c) in its plainest form: a control whose only job is to choose between two views has nothing to offer a host that is already showing one of them. This is the one place `/embed` is used for a pane, and it does not contradict D230's rejection of `/embed` for a FILE target: that objection is that `/embed` nests the target one iframe deeper, out of the annotation layer's reach, which matters only for a target the annotation layer *points at* — no pin on a file listing would mean anything to the agent. **The corollary is enforced, not documented:** while the pane is the embedded listing the annotate affordance is **hidden** and `annSetMode` refuses to arm whoever asks — guarded in that one function rather than at each caller, because arming has four entry points (the button, the boot default, the `annmode` param, the app menu's exit action), `annmode` defaults ON, and the pane resolves asynchronously, so a check per caller is the check a fifth caller forgets. Rejected: the previous behaviour, which was to `throw` for a folder with no app entry — a permanent error panel beside a working chat, for the ordinary case of a folder that is not an app.
  - **The system prompt** (`_split_system_prompt`) has three shapes matching the three pane shapes, and is decided **per run, never cached**, so a folder being scaffolded into starts being described as a project the moment it becomes one. An **app folder** keeps the project wording (its HTML is an app fused-render serves through the `runPython` bridge; naming fused-render here rather than leaving it to the user's own `CLAUDE.md`, which we do not own — the D216 reliability argument). An **ordinary folder** gets the folder-scoping instruction and an honest description of the pane: it is *fused-render's own file browser*, our UI over their folder, never a thing to edit, and `app_state` reports the **browser**, not the folder. A **file** says whose page the pane is and that the viewer is never to be edited. The **composer's placeholder** names the same three kinds and is set from the same resolution the pane already performs (stat's `is_dir`, then whether an entry html resolves) — *"Ask Claude about this **project** / **folder** / **file**…"*, with the markup shipping the kind-free *"Ask Claude…"* until stat answers; it was hardcoded to "this project", which was the wrong noun for an ordinary folder and for all 47 file keys, and the rule is the prompt's rule: the UI does not claim a kind the target does not have. All three carry the **app_state disclosure**, for D230's reason: an un-announced tool is a tool that never gets called. Saying "this is a fused-render project" over `~/Downloads` is rejected as a lie that costs something — it invites the agent to hunt for a bridge that is not there and to read a folder of PDFs as a codebase.
...
- **PT-10** **Mode switcher (shell, preview header):** rendered only when `templates.length > 1`, right side of the preview header bar. Its contents are exactly the resolved list — the **available width is never an input** to it, so a mode cannot appear or vanish as a divider is dragged (PT-15). **Icon-only buttons**, mode name via native `title` tooltip, active mode in accent color. When an entry's `icon` is `null`, the shell renders a placeholder: the first letter of the mode name in a small rounded box. The `.html` Rendered|Source pair is **not a special case**: it is the ordinary mode list `["_render", "code"]` (PT-12) riding this same switcher — `_render` gets a shell-baked eye icon (sentinels have no folder to ship `icon.svg`); `code` gets its real folder icon. The `_listing` sentinel likewise gets a shell-baked list icon (D81).
...
and a second staler way in was not worth keeping. **Since D232 the mode is
...
(`claudeComments`/`claudeReturn`) lived only in the plain chat template D232
...
**The whole Send-to-Claude round trip below is DEAD CODE as of D232, and it is
...
D232 deleted; the surviving chat (PT-14/PT-16) never grew it. So the outbound leg
...
The return leg was the chat's — the half D232 deleted, per the note above; what
...
- **HV-7** Interactivity — plain shell navigation via `window.top.location`
  ...
  **The `_mode` literal in a resume link is load-bearing, and D230/D232 proved
  ...
  wrong view, with the session dropped and nothing said. D230 caused exactly that
  by moving the chat off the file keys, and D232's rename would have caused it
...
- **HV-8** Comments render **read-only** (content, created/updated time,
  ...
  as of **D232, entirely INERT — no row is a link.** A tombstoned entry (an
  ...
  lookup (§17). D230 deregistered `annotate` from all 66 keys, which made that
  URL an unknown mode value — so the click silently opened the file's default
  view with the comment id dropped (PT-9). D232 removed the link rather than
...
- **MD-2** **Registry.** `.md`/`.markdown` are `["markdown", "code",
  ...
  gives a note a chat about itself. **D230** rebound this key from the plain chat
  to the split one (`claude_split`) because a note is a FILE; **D232** deleted the
  ...
  §33 — D230, which made `git` directory-only and put a folder chat beside it,
  and D232, which folded the two chat entries this key briefly carried into the
...
- **GT-2** **Registry bindings — the universal `/` directory key, and nothing
  else (D230).** `git` sits in `"/": ["_listing", "app", "claude",
  "versions", "git", "graph", "zarr_aoi"]` (one chat entry since D232, not two), where `_listing` stays the
  ...
  same peer exclusion `git/condition.py` already applies to app folders. **D232
  did not touch this**, and it is the half of D230 that stands: the chat is one
