...
- **RH-3** Returns a Promise. Resolves with the deserialized return value; rejects with a structured error `{ type, message, traceback, where }` on Python exception, missing file, missing `main` function, or timeout. `traceback` starts at user code (runner frames trimmed, D72); `where` = `{file, line, func, source}` of the deepest frame in the user's own script, or `null` when the error never touched it.
