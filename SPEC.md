...
- **LSN-3** Tracking upserts when the shell query has a param **other than
  ...
  bug D323 records: opening the file preview's companion sidebar STARTED that
...
- **LSN-12** **SOME PARAMS MAY NEVER REACH A SIDECAR, and `_side` is the first**
  (D323). The file preview's companion sidebar — whether it is up and which
