...
reached through `hf_auth.token()` and derived nowhere else in this app (D381):
...
place as it nears expiry. An unfiltered query **over-fetches 4x (capped at 200) and truncates
...
placed in that environment** (D381): a worker imports `huggingface_hub` and so
