...
def test_gated_directory_peers_follow_the_listing():
    ...
    assert error is None
    assert got == ["_listing", "versions", "git", "graph", "zarr_aoi"]
...
def test_unmapped_file_empty_and_plain_dir_lists():
    ...
    assert modes("/x/somedir", is_dir=True) == (["_listing", "versions", "git", "graph", "zarr_aoi"], None)
    assert modes("/x/my.data", is_dir=True) == (["_listing", "versions", "git", "graph", "zarr_aoi"], None)
    assert modes("/", is_dir=True) == (["_listing", "versions", "git", "graph", "zarr_aoi"], None)
...
def test_registry_drops_zarr_template_and_sentinel_keys():
    ...
    assert registry["/"] == ["_listing", "versions", "git", "graph", "zarr_aoi"]
...
def test_plain_dir_with_store_marker_gates_true(tmp_path, marker):
    ...
    assert modes(str(store), is_dir=True) == (["_listing", "versions", "git", "graph", "zarr_aoi"], None)
    assert _zarr_condition_main()(str(store)) is True
    cond, err = conditions(str(store))
...
def test_plain_dir_without_markers_gates_false(tmp_path):
    ...
    assert entries[1]["mode"] == "versions" and entries[1].get("conditional") is True
