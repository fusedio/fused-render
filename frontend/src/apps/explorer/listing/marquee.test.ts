// Rubber-band selection, as arithmetic: the box two points make, which rows it
...
describe("autoScrollStep", () => {
  ...
  test("the middle of the listing does not scroll", () => {
    expect(autoScrollStep(300, view)).toBe(0);
    expect(autoScrollStep(140, view)).toBe(0);
  });

  test("the bottom edge scrolls down, the top edge up", () => {
    expect(autoScrollStep(495, view)).toBeGreaterThan(0);
    expect(autoScrollStep(105, view)).toBeLessThan(0);
  });

  test("deeper into the edge zone is faster", () => {
    expect(autoScrollStep(499, view)).toBeGreaterThan(autoScrollStep(485, view));
    expect(autoScrollStep(101, view)).toBeLessThan(autoScrollStep(115, view));
  });
  ...
  test("past the edge holds at full speed, it does not accelerate away", () => {
    ...
    const atEdge = autoScrollStep(500, view);
    expect(autoScrollStep(900, view)).toBe(atEdge);
    expect(autoScrollStep(-900, view)).toBe(-atEdge);
  });
});
