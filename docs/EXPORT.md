...
| `fused.job(...)` | ⚪️ | a **no-op stub**: the handle and all its methods exist and resolve, so a page that reports progress exports and runs unchanged — there is simply no download manager on a hosted page to report to (SPEC AC-14). Unlike `fused.ai`, it does **not** block export: progress reporting is decoration, not data. |
