...
def test_the_session_picker_is_named_with_words_not_with_machinery(
        client, projects_dir, state_dir, tmp_path):
    """This reader filtered NOTHING, so the picker's row for a chat started from
    the fused-render Claude page was called `<live-app-state>` — the page's own
    wire, addressed to the model, quoted back at the human as the name of their
    conversation. Same stripper as the Tasks list now (tasks_store), so the two
    surfaces cannot disagree about what a session is called."""
    proj = tmp_path / "proj"
    proj.mkdir()
    _write(projects_dir, "proj", "s1", [
        {"cwd": str(proj),
         **_user(records.prefixed(records.APP_STATE, records.PANE_SHOT,
                                  records.PROSE), "2026-08-16T08:00:00Z")},
    ], mtime=STALE)
...
def test_the_picker_reads_past_machinery_and_past_a_subagents_brief(
    ...
    nor `isSidechain`, so Claude Code's own local-command caveat and a subagent's
    task brief could both name a session the user never typed them in."""
    proj = tmp_path / "proj"
    proj.mkdir()
    _write(projects_dir, "proj", "s1", [
        {"cwd": str(proj), "isMeta": True,
         **_user("Caveat: The messages below were generated by the user while "
                 "running local commands.", "2026-08-16T08:00:00Z")},
        {"cwd": str(proj), "isSidechain": True,
         **_user("You are a read-only investigator. Map every caller of "
                 "_parse_head and report file:line.", "2026-08-16T08:00:01Z")},
        {"cwd": str(proj), **_user(records.SLASH_COMMAND, "2026-08-16T08:00:02Z")},
        {"cwd": str(proj), **_user("now fix the parser", "2026-08-16T08:00:03Z")},
    ], mtime=STALE)
    assert _get(client)[0]["name"] == "now fix the parser"
