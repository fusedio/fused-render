...
VIEWER_TOKEN = "UDF_Fused_App_File"
...
def canvas_name(app_id: str) -> str:
...
def _remote_prefix(handle: str, app_id: str) -> str:
    return f"fd://{handle}/{REMOTE_SUBDIR}/{app_id}/"
...
def _wrapper_source(slug: str, s3_uri: str, name: str) -> str:
    return (
        f'"""Shared page for the fused-render app {name!r} — generated by '
        'fused-render, do not hand-edit.\n\n'
        "Rendering is delegated to the Fused file UDF Fused_App_File, the same\n"
        "viewer the workbench opens a .fused file with. Regenerate by sharing again.\"\"\"\n\n"
...
def _canvas_zip(name: str, slug: str, title: str, s3_uri: str) -> bytes:
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("canvas.toml", _canvas_toml(name, slug, title))
        zf.writestr(slug + ".py", _wrapper_source(slug, s3_uri, title))
    return buf.getvalue()
...
def publish(req: dict) -> dict:
    app_id, name, local = req["app_id"], req["name"], req["file"]
    ...
    api.import_collection_toml_zip(collection_id, _canvas_zip(cname, slug, name, s3_uri))
    ...
    if canvas_id:
        try:
            ...
            if "not found" not in str(exc).lower():
                raise
    try:
        fused.api.delete(_remote_prefix(_handle(), req["app_id"]), max_deletion_depth="unlimited")
