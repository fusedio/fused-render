...
// which this module outlived. Nothing MOUNTS on the recap answer, so the cost
...
test("a read that HANGS costs one budget and leaves the default standing", async () => {
  ...
  // The fold was on the whole way through: the default IS the answer here.
  expect(seen[seen.length - 1]).toBe(true);
  expect(chatRecapEnabledNow()).toBe(true);
...
test("a read that is merely SLOW is not abandoned in favour of asking again", async () => {
  ...
  answer = () =>
    new Promise((done) => setTimeout(() => done({ chat: { recap: false } }), 20));
