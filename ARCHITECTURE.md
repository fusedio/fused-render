...
│   │       ├── shell.py        # "/", "/view/{path}", "/embed/{path}" (serves the built React shell)
...
**Split preview pane (SPEC FS-9..FS-15, D185):** `Listing.tsx` optionally splits into list + preview pane, toggled from the search row, default off. The pane's file case builds the **same** iframe src as `TemplatePreview` (`/render?path=<default template>&_file=<file>`, SPEC PT-2/PT-8) — one URL construction, so a file previews identically in the pane and full-screen — including PT-9's tail: it picks the first unconditional entry, and for an **all-conditional** list resolves the gates (`resolveConditions`) and shows the first allowed one rather than falling through to the metadata card, matching `Preview.defaultTemplate`'s `templates[0]` fallback; the same PT-12 sentinel filter runs first so a dangling `path: null` entry can never build a `path=null` render URL. The directory case is a peek (single-app embed, else a read-only mini child list), not a nested `<Listing>`. A draggable divider clamps to 220 px minimum list width / 65 % maximum pane width through one `clampPaneWidth(containerW, width)` helper shared by the drag and the default measurement — two independent ceilings, `containerW * 0.65` and `containerW - LIST_MIN_W`, since the fraction alone does not hold the list's floor on a narrow container (65 % of 600 px leaves the list 210 px); `.listing-scroll` inside a split with a pane also carries `min-width: 220px` so a *window resize* under an already-sized pane cannot squeeze the table, and `PANE_MIN_W` is applied last so a container too small for both minimums keeps the pane's floor and scrolls the list. The pane defaults to 50 % of the split container measured in a `useLayoutEffect` (`PANE_DEFAULT_FRAC`; the old fixed `PANE_FALLBACK_W = 420` is now only the unmeasurable-container fallback). That measurement is a **one-time default** (owner call, SPEC FS-12) — it fills `pane.width` in and thereafter behaves like any other pixel width, so the pane does not re-track the container on a window resize. Because the measured value is indistinguishable from a dragged one in state, a `paneSized` ref carries the provenance: true when the width was restored from `panew` or set by a drag that actually moved the divider, and the two `savePaneState` callers (toggle, divider pointer-up) pass `null` unless it is set — otherwise a later toggle would write the measured default into `panew` as if the user had chosen it, permanently freezing that window's arithmetic. The ref resets with the component, which remounts per `fsPath`, matching `panew`'s per-folder scope. `savePaneState` writes the two keys **independently** — `pane` follows the toggle, `panew` is only rewritten (or cleared when unsized) — so toggling the pane off no longer deletes a dragged width that the mount would otherwise have to re-measure on the next open. **Visibility and width persist differently** (SPEC FS-13): width is viewstate-only (`panew` in the per-path store that already holds the folder's sort), so a pixel drag never enters a bookmark (SPEC SB-2); visibility rides the **URL** as a sticky `?preview=true` *and* viewstate (`pane=1`), with `resolvePane()` treating a present `?preview` as authoritative and an effect reflecting a viewstate-restored pane back into the URL so the two cannot disagree. `navigate()` in `lib/router.ts` carries the param onto **directory** targets only, and `Breadcrumb.tsx`'s `navigatePreservingMode` carries it beside `_mode`; **file** targets drop it deliberately, since `preview` is unreserved and runtime.js's D72 ancestor-climb would expose it to the template iframe a file view always hosts. Residual, documented in FS-13: a template rendered *inside the pane* does see it, because the listing URL legitimately carries it then. Loading states are skeleton shimmer, and the pane is never on the listing's critical path — a stalled pane leaves the list interactive. **The pane also owns the plain row click** (SPEC FS-15): `onRowClick` navigates only when `!pane.on`, and `onRowDoubleClick` navigates only when `pane.on` — so exactly one of the two handlers opens a row, chosen by pane state, and neither branch needs a click-delay timer (the first click of a double-click selects, and the navigation supersedes the pane fetch it started). Shift/Mod clicks and `Enter` are outside this switch and behave identically with the pane on or off. This replaces the deleted `templates/preview/` folder, which was a vanilla-JS fork of this component (D185); `preview` is gone from the built-in `"/"` list, now `["_listing", "graph", "zarr_aoi"]`.
