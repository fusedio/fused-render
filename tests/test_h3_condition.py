...
def test_plain_parquet_rejected(method, parquet):
...
def test_empty_parquet_rejected(method, parquet):
    p = parquet("SELECT 1::BIGINT AS hex WHERE false")
    assert method(p) is False


def test_missing_file_rejected(method, tmp_path):
    assert method(str(tmp_path / "nope.parquet")) is False


def test_non_parquet_bytes_fail_closed(method, tmp_path):
    p = tmp_path / "bad.parquet"
    p.write_text("not parquet at all")
    assert method(str(p)) is False
