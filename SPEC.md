...
- **FS-16** **Opening a folder auto-selects its first PAGE, else its first entry**, so the pane opens showing something. With the pane on and nothing else claiming the selection, the listing walks the **rendered order** the first time the folder's listing settles and selects the first **page** row — an `.html`/`.htm` FILE — falling back to the plain first row, file or directory, when the folder has none (D263, superseding D240's answer; D240's other halves — a directory is an ordinary candidate, and the decision reads no URL — stand). Rendered order both ways, so the choice follows the active sort and is the qualifying row nearest the top of the table the user is looking at; deliberately **not** a favoured `index.html`, which could select a row scrolled off screen and would need a tie-break of its own the moment a folder held two. The row **kind** is checked, not the name — a directory called `build.html` (an exported site tree) is not a page. `.htm` counts HERE and not in the app gate (`lib/folder-app.ts`, mirroring `app_listing.app_entry`), and the split is the answer to each question rather than a drift: the app gate makes a claim the server also makes and the two must agree to the letter, while this one picks a preview row where the registry renders `.htm` and `.html` identically and being wrong costs a keystroke. Otherwise the pane opened on the folder's own **self target** (FS-11), whose default state is the `Select a file to preview.` hint: a pane that opens empty asks the user to do the obvious thing before it will do anything at all, on a view they opened precisely to see what is inside. **A directory is previewed like any other row** (D240, overturning the original skip-directories rule): selecting one shows it in the pane as an embedded listing (FS-11), not a navigation, so landing on it is as harmless as landing on a file — and with dirs-first sorts it is the row the eye actually lands on. Only an **empty folder** keeps the self target and its hint, unchanged.
